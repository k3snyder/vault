...
log.debug('PDF.js worker configured successfully');

// Make debug function available globally
window.debugPDFSelection = function () {
  return debugSelectionStyles();
};
...
const injectSelectionStyles = () => {
  ...
  log.debug('PDF selection styles injected');
};
...
export async function loadPDF(urlOrPath, filePath = null) {
  log.debug(`Loading PDF from: ${urlOrPath}`);
  log.debug('Worker configured at:', pdfjsLib.GlobalWorkerOptions.workerSrc);
  ...
  try {
    ...
    if (filePath && invoke) {
      log.debug('Reading PDF file via Tauri API...');
      log.debug('File path:', filePath);

      try {
        // Read the file as base64
        log.debug('Invoking read_file_base64 with path:', filePath);
        const base64Data = await invoke('read_file_base64', { path: filePath });
        log.debug('Base64 data received, length:', base64Data.length);
        ...
        pdfData = { data: bytes };
        log.debug('PDF file read successfully, size:', bytes.length);
      } catch (readError) {
        ...
        try {
          log.debug('Trying alternative file reading approach...');
          ...
          log.debug('Reading via read_image_as_base64 with relative path:', relativePath);
          const base64Data = await invoke('read_image_as_base64', { path: relativePath });
          log.debug('Alternative read successful, base64 length:', base64Data.length);
          ...
          pdfData = { data: bytes };
          log.debug('PDF converted to bytes, size:', bytes.length);
        } catch (altError) {
      ...
      log.debug('Using URL approach (no file path or invoke not available):', urlOrPath);
      pdfData = { url: urlOrPath };
    }
    ...
    loadingTask.onProgress = (progress) => {
      log.debug(`Loading progress: ${progress.loaded}/${progress.total}`);
    };

    currentPDF = await loadingTask.promise;

    log.debug(`PDF loaded successfully: ${currentPDF.numPages} pages`);
...
export async function renderAllPages(container) {
  ...
  log.debug(`Rendering all ${currentPDF.numPages} pages`);
...
async function renderPage(pageNum, container) {
  log.debug(`Rendering page ${pageNum}`);
  ...
  try {
    ...
    log.debug(`Canvas rendered for page ${pageNum}`);
    ...
    log.debug(`Text content retrieved for page ${pageNum}: ${textContent.items.length} text items`);
    ...
    log.debug(`Text layer rendered for page ${pageNum}`);
    ...
    textLayerDiv.addEventListener('mouseup', () => {
      ...
      if (selectedText) {
        ...
        if (selection.rangeCount > 0) {
          ...
          log.debug('🔵 Selection rectangles:', rects.length);
        }
      }
    });
...
function renderHighlightsForPage(pageNum, pageDiv) {
  ...
  log.debug(`Rendering ${pageHighlights.length} highlights for page ${pageNum}`);
...
export async function zoomIn(container) {
  currentScale *= 1.2;
  log.debug(`Zooming in to scale: ${currentScale}`);
...
export async function zoomOut(container) {
  currentScale /= 1.2;
  log.debug(`Zooming out to scale: ${currentScale}`);
  await renderAllPages(container);
}
...
export function setHighlights(newHighlights) {
  ...
  log.debug('Highlights loaded:', Object.keys(highlights).length, 'pages with highlights');
}
...
export function addHighlight(pageNum, highlight) {
  ...
  log.debug(`Added highlight to page ${pageNum}: "${highlight.text.substring(0, 50)}..."`);
}
...
export function debugSelectionStyles() {
  log.debug('🔵 PDF Selection Debug Info:');
  ...
  log.debug('PDF Viewer stylesheet loaded:', !!pdfViewerStyles);

  // Check text layers
  const textLayers = document.querySelectorAll('.textLayer');
  log.debug(`Found ${textLayers.length} text layers`);

  textLayers.forEach((layer, index) => {
    const computed = window.getComputedStyle(layer);
    const spans = layer.querySelectorAll('span');
    log.debug(`Text layer ${index}:`, {
    ...
    if (spans.length > 0) {
      ...
      log.debug(`First span in layer ${index}:`, {
  ...
  log.debug('Test ::selection styles:', {
  ...
  log.debug(`Pages with 'selecting' class: ${selectingPages.length}`);
