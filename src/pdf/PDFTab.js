...
export class PDFTab {
  ...
  createToolbar() {
    ...
    toolbar
      .querySelector('.pdf-zoom-out-btn')
      .addEventListener('click', () => this.handleZoomOut());
    toolbar.querySelector('.pdf-zoom-in-btn').addEventListener('click', () => this.handleZoomIn());
    toolbar
      .querySelector('.pdf-highlight-btn')
      .addEventListener('click', () => this.highlightSelection());
    toolbar
      .querySelector('.pdf-undo-btn')
      .addEventListener('click', () => window.dispatchEvent(new CustomEvent('pdf-undo-highlight')));
    toolbar
      .querySelector('.pdf-redo-btn')
      .addEventListener('click', () => window.dispatchEvent(new CustomEvent('pdf-redo-highlight')));
    toolbar
      .querySelector('.pdf-extract-btn')
      .addEventListener('click', () => this.extractHighlights());
    toolbar
      .querySelector('.pdf-clear-highlights-btn')
      .addEventListener('click', () => this.clearAllHighlights());
    toolbar
      .querySelector('.pdf-intelligence-btn')
      .addEventListener('click', () => this.openIntelligencePanel());
  ...
  async initializePDF() {
    try {
      ...
      this.fullPath = vaultPath ? `${vaultPath}/${this.pdfPath}` : this.pdfPath;
      ...
      this.pageChangeHandler = (pageNumber) => {
        if (Number.isInteger(pageNumber) && pageNumber > 0) {
          this.currentPage = pageNumber;
          this.updatePageCounter();
        }
      };
      this.viewerWrapper.onPageChange(this.pageChangeHandler);
  ...
  clearAllHighlights() {
    ...
    const confirmClear = confirm(
      'Are you sure you want to clear all highlights? This action can be undone with Cmd+Z.',
    );
    if (confirmClear) {
  ...
  async extractHighlights() {
    try {
      ...
      if (selection && selection.toString().trim() && selection.rangeCount > 0) {
        ...
        const pdfContainer =
          range.commonAncestorContainer.closest?.('.pdf-viewer-container') ||
  ...
  destroy() {
    ...
    if (this.viewerWrapper) {
      if (this.pageChangeHandler) {
        this.viewerWrapper.offPageChange(this.pageChangeHandler);
  ...
  updateCurrentPage() {
    ...
    for (let i = 0; i < pages.length; i++) {
      ...
      if (
        pageRect.top < containerRect.top + containerRect.height / 2 &&
