...
export function createBotckyContextPayload({
  ...
  const safeVaultInfo = vaultInfo && typeof vaultInfo === 'object' ? vaultInfo : {};
  const resolvedVaultPath = vaultPath || safeVaultInfo.path || '';
  const resolvedVaultId = vaultId || safeVaultInfo.id || resolvedVaultPath;
...
export function collectBotckyContext(source = {}) {
  ...
  const context =
    safeSource.context && typeof safeSource.context === 'object'
      ? { ...safeSource.context, ...safeSource }
      : safeSource;
  ...
  if (
    hasExplicitCurrentFolder &&
    !String(context.current_folder || context.currentFolder || '').trim()
  ) {
