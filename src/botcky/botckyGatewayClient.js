...
export const BOTCKY_OFFLINE_MESSAGE = 'Botcky is offline';
...
export function gatewayWsUrl(endpoint, sessionId, afterSeq, apiKey) {
  ...
  url.pathname = `${url.pathname.replace(/\/+$/, '')}/gateway/ws/chat`.replace(
    '//gateway/ws/chat',
    '/gateway/ws/chat',
  );
...
export function botckyGatewayOfflineError(cause) {
  const error = new Error(BOTCKY_OFFLINE_MESSAGE);
  error.cause = cause;
  error.offline = true;
  return error;
}
...
export class BotckyGatewayNativeClient {
  constructor({
    endpoint = DEFAULT_BOTCKY_GATEWAY_ENDPOINT,
    sessionId,
    apiKey = null,
  ...
  connect({ afterSeq } = {}) {
    ...
    this.socket = new this.WebSocketCtor(
      gatewayWsUrl(this.endpoint, this.sessionId, afterSeq, this.apiKey),
    );
    this.socket.addEventListener?.('open', () =>
      this.emit({ type: 'socket.open', session_id: this.sessionId }),
    );
    this.socket.addEventListener?.('message', (event) => this.handleMessage(event.data));
    this.socket.addEventListener?.('error', (event) =>
      this.emit({ type: 'socket.error', error: event?.message || 'WebSocket error' }),
    );
    this.socket.addEventListener?.('close', (event) =>
      this.emit({ type: 'socket.close', code: event?.code, reason: event?.reason }),
    );
    return this.socket;
  }
  ...
  async deleteAllSessions({ platform = 'vault', status = null } = {}) {
    ...
    const ids = sessions.map((session) => session?.id || session?.session_id).filter(Boolean);
  ...
  fetchHistory(sessionId = this.sessionId) {
    return this.fetchJson(
      `/gateway/chat/sessions/${encodeURIComponent(sessionId)}/messages?include_events=true`,
    );
  }
  ...
  async fetchJson(path, init = {}) {
    ...
    let response;
    try {
      response = await this.fetchImpl(`${this.endpoint}${path}`, { ...init, headers });
    } catch (error) {
      throw botckyGatewayOfflineError(error);
    }
    if (!response.ok)
      throw new Error(`Botcky Gateway ${response.status}: ${await response.text()}`);
...
function isDefaultLocalGateway(endpoint) {
  try {
    ...
    return (
      isLocalHost(parsed.hostname) && parsed.port === '7110' && isGatewayRootPath(parsed.pathname)
    );
  } catch {
    return false;
  }
