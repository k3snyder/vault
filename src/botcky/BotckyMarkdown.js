  ...
  while (index < lines.length) {
    ...
    if (line.startsWith('```')) {
      ...
      blocks.push(
        React.createElement(
          'pre',
          { key: key++, className: 'botcky-code-block' },
      ...
      while (index < lines.length && isUnorderedListLine(lines[index])) {
        ...
        items.push(
          React.createElement(
            'li',
            { key: items.length },
            renderInlineMarkdown(item, `ul-${key}-${items.length}`),
          ),
        );
        index += 1;
      }
      ...
      while (index < lines.length && isOrderedListLine(lines[index])) {
        ...
        items.push(
          React.createElement(
            'li',
            { key: items.length },
            renderInlineMarkdown(item, `ol-${key}-${items.length}`),
          ),
        );
        index += 1;
      }
    ...
    blocks.push(
      React.createElement(
        ...
        paragraphLines.flatMap((paragraphLine, lineIndex) => {
          ...
          return lineIndex === 0
            ? inline
            : [React.createElement('br', { key: `br-${lineIndex}` }), ...inline];
        }),
      ),
    );
  }
...
function normalizeCommonModelMarkdown(markdown) {
  ...
  const otherNumberMarkers = lines.filter((line) =>
    /^\s{0,3}(?:[2-9]|\d{2,})\.\s+/.test(line),
  ).length;
  ...
  return lines.map((line) => line.replace(/^(\s{0,3})1\.\s+/, '$1- ')).join('\n');
}
...
export function renderInlineMarkdown(text, keyPrefix = 'inline') {
  ...
  while ((match = tokenPattern.exec(value)) !== null) {
    if (match.index > lastIndex) {
      nodes.push(
        ...textNodesWithBreaks(value.slice(lastIndex, match.index), `${keyPrefix}-t-${tokenIndex}`),
      );
    }
    ...
    if (match[1] !== undefined) {
      nodes.push(
        React.createElement(
          'code',
          { key: `${keyPrefix}-code-${tokenIndex}`, className: 'botcky-inline-code' },
          match[1],
        ),
      );
