...
async function parseResponse(response) {
  ...
  if (!response.ok) {
    const error = new Error(
      body?.error || body?.message || `Botcky task request failed: ${response.status}`,
    );
...
export function buildBotckyTaskRequest({
  title,
  instructions,
...
export class BotckyTaskClient {
  ...
  createTask(options = {}) {
    return this.request('/gateway/tasks', {
      method: 'POST',
      body: buildBotckyTaskRequest({
        ...options,
        context: options.context || this.contextProvider(),
      }),
    });
  }
  ...
  cancelTask(taskId, overrides = {}) {
    ...
    return this.request(`/gateway/tasks/${encodeURIComponent(taskId)}/cancel`, {
      method: 'POST',
      body: buildBotckyTaskRequest({
        context: { ...this.contextProvider(), ...overrides },
        task: { id: taskId },
      }),
    });
  }
