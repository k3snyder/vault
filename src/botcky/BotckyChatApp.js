...
const MemoizedBotckyMarkdown = React.memo(
  BotckyMarkdown,
  (previous, next) => previous.content === next.content,
);

export function botckyUserFacingError(error) {
  const message = error?.message || String(error || '');
  if (
    error?.offline ||
    isBotckyOfflineMessage(message) ||
    isBotckyStartupPreflightMessage(message)
  ) {
    return BOTCKY_OFFLINE_MESSAGE;
  }
  return message || 'Botcky failed';
}

function botckyErrorEvent(error) {
  return { type: 'error', error: botckyUserFacingError(error) };
}

function isBotckyOfflineMessage(message) {
  return [
    'Load failed',
    'Failed to fetch',
    'NetworkError',
    'Could not connect to the server',
    'Botcky Gateway socket is not connected',
    'WebSocket error',
  ].some((offlineMessage) => message.includes(offlineMessage));
}

function isBotckyStartupPreflightMessage(message) {
  return message.startsWith('Missing required Botcky context:');
}
...
function BotckyThinkingIndicator() {
  return React.createElement(
    ...
    React.createElement(
      'span',
      { className: 'botcky-thinking-dots', 'aria-hidden': 'true' },
...
export function BotckyChatApp({
    ...
    async function startSession() {
      try {
        ...
        reconcileHistory(nativeClient, stateRef, setState, () => disposed).catch((error) => {
          if (!disposed) {
            setState((prev) => reduceBotckyEvent(prev, botckyErrorEvent(error)));
          }
        });
  ...
  React.useEffect(() => {
    ...
    let disposed = false;
    const intervalMs = state.activeRunId
      ? ACTIVE_RUN_LIVENESS_INTERVAL_MS
      : IDLE_LIVENESS_INTERVAL_MS;
    const tick = async () => {
      if (disposed) return;
      try {
        client.ping();
  ...
  const sessionOptions = React.useMemo(() => {
    ...
    if (!seen.has(sessionId)) {
      options.unshift({
        id: sessionId,
        label: options.length === 0 ? 'Session 1' : 'Current Session',
      });
    }
    return options;
  }, [sessions, sessionId]);
  const selectedSessionLabel =
    sessionOptions.find((option) => option.id === sessionId)?.label || 'Session 1';
  const transcriptItems = React.useMemo(
    () => buildBotckyTranscriptItems(state.messages),
    [state.messages],
  );
  ...
  async function archiveSession() {
    ...
    try {
      ...
      await Promise.all(
        targetSessionIds.map((targetSessionId) => client.archiveSession(targetSessionId)),
      );
  ...
  async function exportCurrentThread() {
    ...
    const content = buildBotckyExportMarkdown({
      ...
      sessionId,
      sessionTitle:
        sessionOptions.find((option) => option.id === sessionId)?.label || 'Botcky Session',
    });
  ...
  function renderMessageActions(message) {
    ...
    return React.createElement(
      'div',
      { className: 'botcky-message-actions' },
      React.createElement(
        'button',
        {
          type: 'button',
    ...
    return React.createElement(
      ...
      React.createElement(
        'button',
        {
          ...
          onClick: (event) =>
            active ? handleRemoveActiveNoteContext(note, event) : handleRemoveContext(note, event),
  ...
  return React.createElement(
    'div',
    { className: 'botcky-native-chat', 'data-testid': 'botcky-native-chat' },
    React.createElement(
      'div',
      { className: 'botcky-toolbar' },
      React.createElement(
        'div',
        { className: 'botcky-toolbar-title' },
      ...
      state.activeRunId &&
        React.createElement(
          'button',
          { type: 'button', className: 'chat-toolbar-btn botcky-toolbar-btn', onClick: cancelRun },
          'Cancel',
        ),
    ...
    React.createElement(
      'div',
      { className: 'botcky-transcript', ref: transcriptRef },
      transcriptItems.map((item) =>
        item.type === 'date'
          ? React.createElement(
              'div',
              { key: item.key, className: 'botcky-date-separator' },
              formatThreadDateTime(item.timestamp),
            )
          : React.createElement(
              'div',
              { key: item.message.id, className: `botcky-message ${item.message.role}` },
              React.createElement(
                'div',
                { className: 'botcky-message-stack' },
                React.createElement(
                  'div',
                  { className: 'botcky-message-bubble' },
                  React.createElement(
                    'div',
                    { className: 'botcky-message-content' },
                    item.message.role === 'assistant'
                      ? item.message.streaming
                        ? React.createElement(BotckyThinkingIndicator)
                        : React.createElement(MemoizedBotckyMarkdown, {
                ...
                item.message.role === 'assistant' &&
                  !item.message.streaming &&
                  renderMessageActions(item.message),
              ),
            ),
      ),
      state.toolCalls.map((tool, index) =>
        React.createElement(
          'div',
          {
            key: tool.id || tool.call_id || index,
            className: `botcky-card tool ${tool.status || ''}`,
          },
          React.createElement(
            'strong',
            null,
            `Tool: ${tool.name || tool.tool_name || tool.toolName || 'unknown'}`,
          ),
          React.createElement(
            'pre',
            null,
            JSON.stringify(tool.result || tool.arguments || tool.input || {}, null, 2),
          ),
        ),
      ),
      state.tasks.map((task) =>
        React.createElement(
          'div',
          { key: task.id, className: `botcky-card task ${task.status || ''}` },
      ...
      state.approvals.map((approval, index) =>
        React.createElement(
          'div',
          {
            ...
            className: `botcky-card approval ${approval.status || ''}`,
          },
    ...
    React.createElement(
      'form',
      { className: 'botcky-composer', onSubmit: sendMessage },
      renderContextControls(),
      React.createElement(
        'div',
        { className: 'botcky-composer-input-row' },
...
function isExpectedSocketClose(event = {}) {
  return (
    event.code === 1000 || event.reason === 'reconnect' || event.reason === 'botcky-host-unmount'
  );
}
...
function normalizeContextUi(contextUi = {}) {
  ...
  return {
    ...
    selectedNotes: Array.isArray(contextUi?.selectedNotes)
      ? contextUi.selectedNotes.filter(Boolean)
      : [],
  };
}
...
export async function ensureBotckyStartupSession({
  ...
  if (!startupSessionLocks.has(lockKey)) {
    const startup = createOrReuseStartupSession(nativeClient, context, title).finally(() =>
      startupSessionLocks.delete(lockKey),
    );
...
function startupSessionLockKey(nativeClient, context) {
  return [nativeClient?.endpoint || '', context?.vault_id || context?.vault_path || 'vault'].join(
    '|',
  );
}
...
function sessionTitleText(sessionOrOption = {}) {
  return String(
    sessionOrOption?.label || sessionOrOption?.name || sessionOrOption?.title || '',
  ).trim();
}
...
function sessionTimestamp(session) {
  return (
    validDate(
      session?.created_at || session?.createdAt || session?.updated_at || session?.updatedAt,
    )?.getTime() || 0
  );
}
...
async function handleToolCall(client, event, contextProvider, sessionId) {
  ...
  try {
    ...
    const context = await contextProvider({
      sessionId,
      threadId: sessionId,
      includeNoteContent: needsFullContext,
    });
...
async function invokeForTool(name, args, scope, context) {
  switch (name) {
    ...
    case 'read':
      return invoke('botcky_read_file', {
        request: {
          vault_root: scope.vault_root,
    ...
    case 'search':
      return invoke('botcky_search_files', {
        request: {
          vault_root: scope.vault_root,
          current_folder: scope.current_folder,
    ...
    case 'create':
      return invoke('botcky_create_file', {
        request: {
          vault_root: scope.vault_root,
          current_folder: scope.current_folder,
          path: args.path,
    ...
    case 'update':
      return invoke('botcky_update_file', {
        request: {
          vault_root: scope.vault_root,
          current_folder: scope.current_folder,
          path: args.path,
    ...
    case 'append':
      return invoke('botcky_append_file', {
        request: {
          vault_root: scope.vault_root,
          current_folder: scope.current_folder,
          path: args.path,
    ...
    case 'vault_shell':
    case 'shell':
      return invoke('botcky_run_allowed_command', {
        request: {
          vault_root: scope.vault_root,
          current_folder: scope.current_folder,
          cwd: args.cwd,
    ...
    case 'executor_task':
    case 'task_create':
      return invoke('botcky_build_executor_task_request', {
        input: {
          prompt: args.prompt || args.text || '',
          agent_type: args.agent_type || 'executor',
          context,
        },
      });
    default:
