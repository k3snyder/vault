...
import {
  DEFAULT_BOTCKY_GATEWAY_ENDPOINT,
...
export class BotckyChatHost {
  ...
  async mount(container) {
    ...
    const endpoint = normalizeGatewayBaseUrl(
      this.endpoint || settings.endpoint || DEFAULT_BOTCKY_GATEWAY_ENDPOINT,
    );
    const apiKey = normalizeGatewayApiKey(this.apiKey || settings.api_key || null, endpoint);
    this.root = this.ReactRoot(container);
    this.root.render(
      React.createElement(BotckyChatApp, {
        endpoint,
        apiKey,
