...
class WindowContext {
  ...
  async openVault(path) {
    if (this.vaultPath === path) {
      console.log('WindowContext: Vault already open, skipping duplicate open', path);
      return;
    }
  ...
  on(event, callback) {
    ...
    // Dedup by reference: registering the same callback twice would double-fire
    // the handler and silently grow the listener list across re-init paths.
    if (callbacks.includes(callback)) {
      return;
    }
    callbacks.push(callback);
    console.log(
      `WindowContext: Registered listener for '${event}', total listeners:`,
      callbacks.length,
    );
  }
  ...
  emit(event, data) {
    const callbacks = this.listeners.get(event);
    console.log(
      `WindowContext: Emitting '${event}' to ${callbacks ? callbacks.length : 0} listeners`,
    );
    if (callbacks) {
