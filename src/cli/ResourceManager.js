...
export class ResourceManager {
  ...
  monitorMemoryUsage() {
    ...
    if (current > threshold) {
      if (this.logger) {
        this.logger.warn(
          `ResourceManager: High memory usage detected: ${(current / 1024 / 1024).toFixed(2)}MB`,
        );
      }
  ...
  getCurrentMemoryUsage() {
    ...
    return Array.from(this.resources.values()).reduce(
      (total, resource) => total + resource.estimatedSize,
      0,
    );
  }
  ...
  performCleanup(aggressive = false) {
    ...
    for (const resource of sortedResources) {
      try {
        ...
        if (this.logger) {
          this.logger.debug(
            `ResourceManager: Cleaned resource ${resource.id}, freed ${result.memoryFreed} bytes`,
          );
        }
      } catch (error) {
    ...
    if (this.logger && cleanupResults.resourcesCleaned > 0) {
      this.logger.info(
        `ResourceManager: Cleanup completed - ${cleanupResults.resourcesCleaned} resources, ${(cleanupResults.memoryFreed / 1024 / 1024).toFixed(2)}MB freed`,
      );
    }
  ...
  forceGarbageCollection() {
    if (typeof global !== 'undefined' && global.gc) {
      ...
      if (this.logger && freed > 0) {
        this.logger.info(
          `ResourceManager: Garbage collection freed ${(freed / 1024 / 1024).toFixed(2)}MB`,
        );
      }
