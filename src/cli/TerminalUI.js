  ...
  async initialize() {
    try {
      ...
      if (!this.ghosttyStatus.installed) {
        ...
        this.renderTerminalError(errorMsg);
        return;
      }
  ...
  render() {
    ...
    this.container.innerHTML = `
      <div class="terminal-header">
        ...
        <div class="terminal-version"></div>
    ...
    `;
    const version = this.container.querySelector('.terminal-version');
    if (version) {
      version.textContent = `v${this.ghosttyStatus.version || 'unknown'}`;
    }
...
Terminal powered by Ghostty`;
  ...
  showError(message) {
    ...
    if (status) {
      ...
      this.renderTerminalError(message);
    }
  }
