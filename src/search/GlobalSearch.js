...
export class GlobalSearch {
  ...
  toggle() {
    console.log(
      'GlobalSearch.toggle() called, isVisible:',
      this.isVisible,
      'container:',
      !!this.container,
    );
      ...
      for (const pattern of patterns) {
        ...
        if (pattern.file_path) {
          ...
          const file = files.find((f) => {
            ...
            return (
              fileName === pattern.name || fileName.toLowerCase() === pattern.name.toLowerCase()
            );
          });
        ...
        for (const item of response.items) {
          ...
          results.push({
            note: {
              title:
                content.title ||
                metadata.file_path?.split('/').pop()?.replace('.md', '') ||
                'Untitled',
