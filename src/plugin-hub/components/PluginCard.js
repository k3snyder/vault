...
class PluginCard {
  ...
  renderResourceBars() {
    ...
    if (resources.memory) {
      bars.push(
        this.createResourceBar('Memory', resources.memory.used, resources.memory.limit, 'MB'),
      );
    }
    ...
    if (resources.storage) {
      bars.push(
        this.createResourceBar('Storage', resources.storage.used, resources.storage.limit, 'MB'),
      );
    }
      ...
      if (!isPremium) {
        ...
        if (this.plugin.id === 'pacasdb') {
          ...
          this.context.showToast(
            'A license is required to enable this plugin. Please activate a license in settings.',
            'warning',
          );
        }
        return;
