...
class PacasDBSettings {
  ...
  attachEventListeners() {
    ...
    // Start Trial
    this.element
      .querySelector('[data-action="start-trial"]')
    ...
    // Activate License
    this.element
      .querySelector('[data-action="activate-license"]')
      ?.addEventListener('click', async () => {
    ...
    // Deactivate License
    this.element
      .querySelector('[data-action="deactivate-license"]')
      ?.addEventListener('click', async () => {
    ...
    // Sync Vault
    this.element
      .querySelector('[data-action="sync-vault"]')
      ?.addEventListener('click', async () => {
  ...
  async deactivateLicense() {
    const confirmed = confirm(
      'Are you sure you want to deactivate your license? This will remove premium features.',
    );
    if (!confirmed) return;
    ...
    try {
      ...
      if (connected) {
        ...
        this.context.showToast(
          'Failed to connect to PACASDB server. Make sure it is running on localhost:8000.',
          'error',
        );
      }
    } catch (error) {
