...
const BUNDLED_PLUGINS = {
  pacasdb: {
    ...
    author: 'Vault Team',
    description:
      'Semantic search and cognitive memory for your vault. Enables AI-powered note discovery, related notes, and intelligent context tracking.',
    enabled: false,
    installed: true,
  ...
  'csv-support': {
    ...
    author: 'Vault Team',
    description:
      'View and edit CSV files with a powerful tabular interface. Includes editing, row/column operations, schema inference, AI context generation, and unlimited rows.',
    enabled: true,
    installed: true,
...
export class PluginContext {
  ...
  setupEventHandlers() {
    this.eventHandlers = {
      ...
      'plugin-resource-threshold': (payload) => {
        this.showToast(`${payload.name} is using high ${payload.resource}`, 'warning');
      },
      'plugin-permission-requested': async (payload) => {
        const granted = await this.requestPermissions(payload.plugin, payload.permissions);
        if (!granted) {
  ...
  createReadwiseMarkdown(item) {
    ...
    if (useMetadataFormat) {
      ...
      if (item.category && item.category !== item.source_type)
        ...
        if (useMetadataFormat) {
          ...
          if (highlight.location) {
            const locationUrl =
              ...
              (item.asin
                ? `https://readwise.io/to_kindle?action=open&asin=${item.asin}&location=${highlight.location}`
                : null);
            if (locationUrl) {
  ...
  getFilteredPlugins() {
    ...
    return fuzzySearchPlugins(this.state.installedPlugins, this.state.searchQuery, {
  ...
  getFilteredAvailablePlugins() {
    ...
    return fuzzySearchPlugins(this.state.availablePlugins, this.state.searchQuery, {
            ...
            try {
              ...
              try {
                ...
                if (authResponse.status !== 204) {
                  throw new Error(
                    'Invalid API token. Please check your token at readwise.io/access_token',
                  );
                }
                ...
                while (true) {
                  ...
                  if (!exportResponse.ok) {
                    ...
                    throw new Error(
                      `Failed to fetch page ${pageCount + 1}: ${exportResponse.status}`,
                    );
                  }
                ...
                for (const item of allItems) {
                  // Use allItems instead of data.results
                ...
                this.showToast(
                  `Readwise sync complete! Saved ${savedCount} of ${allItems.length} items.`,
                  'success',
                );
