...
export class BedrockClaudeSDK {
  ...
  buildPrompt(userMessage, contextNotes = [], tagEnhancement = null) {
    ...
    const systemPrompt =
      ...
      'You are a helpful assistant integrated into a local-first notes app. Provide concise, accurate answers. Prefer actionable lists. If context notes are provided, prioritize them over chat history.';
  ...
  async sendMessage(userMessage, contextNotes = [], tagEnhancement = null) {
    ...
    if (settings.headers && Array.isArray(settings.headers)) {
      for (const kv of settings.headers) {
        ...
        if (kv.name.toLowerCase() === 'authorization' || kv.name.toLowerCase() === 'content-type')
          continue;
