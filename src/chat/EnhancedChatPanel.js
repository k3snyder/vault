...
import { createBotckyContextPayload, currentFolderFromPath } from '../botcky/botckyContext.js';
import { createLogger } from '../utils/logger.js';
import { getActiveTab, getCurrentFile } from '../app/workspace.js';
import { refreshFileTree } from '../app/file-tree-view.js';

// Import Tauri API
import { invoke } from '@tauri-apps/api/core';

const log = createLogger('EnhancedChatPanel');
...
export class EnhancedChatPanel {
  constructor() {
    log.debug('🔧 Initializing Enhanced ChatPanel');
    this.container = null;
  ...
  updateContextCharLimit(settings) {
    ...
    log.debug(`🧠 Context character limit set to ${this.contextCharLimit} (tokens: ${maxTokens})`);
  }
  ...
  syncBotckyVaultPath(vaultPath) {
    const normalizedVaultPath =
      typeof vaultPath === 'string' && vaultPath.trim() ? vaultPath.trim() : null;
  ...
  async mount(parentElement) {
    log.debug('📌 Mounting Enhanced ChatPanel');
    ...
    // Initialize providers. Each provider owns SDK-specific formatting and streaming.
    this.providers.openai.sdk = new OpenAIProvider();
    this.providers.gemini.sdk = new GeminiProvider();
    this.providers.bedrock.sdk = new BedrockProvider();
    this.providers.claudeAgent.sdk = new ClaudeAgentProvider();
    ...
    this.auth.onAuthStateChanged = (authenticated) => {
      log.debug('🔐 Auth state changed:', authenticated);
    ...
    this.interface.onSendMessage = async (message) => {
      log.debug('📤 Sending message via', this.currentProvider);
    ...
    log.debug('✅ Enhanced ChatPanel mounted successfully');
  }
  ...
  async initializeProviders() {
    log.debug('🚀 Initializing AI providers...');
    ...
    try {
      ...
      if (settings?.provider !== 'claudeAgent') {
        try {
          const claudeSettings = await invoke('get_ai_settings_for_provider', {
          ...
          log.debug('No Claude Agent settings found, using current settings');
        }
      }
      ...
      if (!botckySettings) {
        try {
          botckySettings = await invoke('get_ai_settings_for_provider', {
            provider: BOTCKY_PROVIDER_ID,
          });
        } catch (e) {
          log.debug('No Botcky Gateway settings found, using native defaults');
        }
      }
      const botckyEndpoint =
        typeof botckySettings?.endpoint === 'string' ? botckySettings.endpoint.trim() : '';
      this.providers.botckyGateway.sdk = null;
      ...
      if (this.isBotckyProviderKey(settings?.provider)) {
        ...
        log.debug('🎯 Using native Botcky chat');
        ...
        if (settings?.endpoint?.includes('/openai/')) {
          console.warn(
            '⚠️ Gemini endpoint uses the legacy OpenAI-compatible path; AI Settings will normalize it on save.',
          );
        }
        log.debug('🎯 Using Gemini SDK');
        ...
        if (settings.endpoint.includes('/openai/')) {
          ...
          console.warn(
            'Please update your Gemini endpoint in AI Settings to: https://generativelanguage.googleapis.com/v1beta/',
          );
          // For now, still use OpenAI SDK which will call the Rust backend
          this.currentProvider = 'openai';
        } else {
          this.currentProvider = 'gemini';
          log.debug('🎯 Detected Gemini API endpoint, using Gemini SDK');
        }
        if (this.currentMode === 'botcky') {
          this.currentMode = 'chat';
          localStorage.setItem('gaimplan-chat-mode', 'chat');
        ...
        log.debug('🎯 Detected Bedrock endpoint, using Bedrock Claude SDK');
        ...
        log.debug('🎯 Detected Bedrock host, using Bedrock Claude SDK');
      } else if (
        settings?.provider === 'claudeAgent' ||
        settings?.endpoint?.includes('anthropic.com')
      ) {
        ...
        log.debug('🎯 Using Claude Agent SDK');
      } else {
        this.currentProvider = 'openai';
        if (this.currentMode === 'botcky') {
          this.currentMode = 'chat';
        ...
        log.debug('🎯 Using OpenAI SDK for endpoint:', settings?.endpoint);
      }
    } catch (error) {
  ...
  buildUI(wrapper) {
    ...
    if (this.showingSettings) {
      ...
      log.debug('💬 Showing enhanced chat interface');
  ...
  initializeChatSessions() {
    ...
    if (activeSessions.length > 0) {
      ...
      this.activeChatSessionId = activeSessions.some(
  ...
  normalizeChatSession(session) {
    ...
    const messages = Array.isArray(session.messages)
      ...
      : [];
    const createdAt =
      this.safeIsoTimestamp(session.createdAt) ||
      this.firstMessageTimestamp(messages) ||
      new Date().toISOString();
    const updatedAt =
      this.safeIsoTimestamp(session.updatedAt) || this.lastMessageTimestamp(messages) || createdAt;
  ...
  sanitizeChatMessage(message) {
    ...
    return {
      ...
      type,
      content:
        message.content === undefined || message.content === null ? '' : String(message.content),
  ...
  createChatSession({ title, messages = [], createdAt = null, status = 'active' } = {}) {
    ...
    const sessionCreatedAt =
      this.safeIsoTimestamp(createdAt) || this.firstMessageTimestamp(normalizedMessages) || now;
    return {
  ...
  getLiveActiveChatMessages() {
    const liveMessages =
      typeof this.interface?.getMessages === 'function' ? this.interface.getMessages() : null;
  ...
  selectAiChatSession(sessionId) {
    ...
    this.saveActiveChatSession();
    if (
      !this.chatSessions.some(
        (session) => session.id === sessionId && session.status !== 'archived',
      )
    ) {
      return;
    }
  ...
  async archiveAiChatSession() {
    ...
    const replacement = this.createChatSession({
      title: this.nextChatSessionTitle(),
      messages: [],
    });
    ...
    this.showNotification(
      session.messages.length > 0 ? 'Chat archived to Chat History' : 'Empty chat archived',
    );
  }
  ...
  createAiChatToolbar() {
    ...
    const newButton = this.createAiToolbarIconButton('plus', 'New session', () =>
    ...
    const deleteButton = this.createAiToolbarIconButton(
      'trash',
      'Delete session',
      () => this.deleteAiChatSession(),
      true,
    );
  ...
  async buildBotckyUI(container) {
    ...
    const { BotckyChatHost } = await import('../botcky/BotckyChatHost.js');
    this.botckyHost = new BotckyChatHost({
      contextProvider: ({ sessionId, threadId, includeNoteContent } = {}) =>
        this.buildBotckyVaultContext({
          sessionId,
          threadId,
          includeNoteContent,
        }),
  ...
  buildBotckyContextUiState() {
    ...
    const selectedNotes = (this.interface?.currentContext || []).filter(
      (note) => note && note.type !== 'active' && note.path !== activePath,
    );
  ...
  async buildBotckyVaultContext({ sessionId, threadId, includeNoteContent = true } = {}) {
    ...
    const activeNoteForContext =
      activeNote && this.interface?.shouldIncludeActiveNoteContext?.(activeNote) !== false
        ? activeNote
        : null;
    ...
    return createBotckyContextPayload({
      ...
      selectedNotes: (this.interface?.currentContext || []).filter(
        (note) => note?.type !== 'active' && note?.path !== activePath,
      ),
  ...
  async getBotckyVaultInfo() {
    ...
    try {
      ...
      return vaultInfo?.path ? vaultInfo : {};
    } catch (error) {
  ...
  async buildCLIUI(container) {
    ...
    if (this.isBuildingCLI) {
      log.debug('CLI Mode: Already building CLI, skipping...');
      return;
    }
    ...
    if (!this.cliContainer) {
      ...
      if (window.windowContext) {
        log.debug('CLI Mode: windowContext exists, getting vault info...');
        try {
          const vaultInfo = await window.windowContext.getVaultInfo();
          log.debug('CLI Mode: vaultInfo from windowContext:', vaultInfo);
          if (vaultInfo && vaultInfo.path) {
            vaultPath = vaultInfo.path;
            log.debug('CLI Mode: Got vault path from windowContext:', vaultPath);
          }
        } catch (error) {
        ...
        log.debug('CLI Mode: Got vault path from window.currentVaultPath:', vaultPath);
      }
      ...
      log.debug('CLI Mode: Using vault path:', vaultPath);
      log.debug('CLI Mode: vaultPath type:', typeof vaultPath);
      log.debug('CLI Mode: vaultPath length:', vaultPath?.length);
      log.debug('CLI Mode: windowContext available:', !!window.windowContext);
      log.debug('CLI Mode: window.currentVaultPath available:', !!window.currentVaultPath);
  ...
  async handleModeToggle(mode) {
    log.debug(`🔄 Toggling mode to: ${mode}`);
    const nextMode =
      mode === 'chat' && this.isBotckyProviderKey(this.currentProvider) ? 'botcky' : mode;
  ...
  async switchToBotckyMode() {
    log.debug('🤖 Switching to native Botcky mode');
    this.currentProvider = BOTCKY_PROVIDER_ID;
    this.providers.botckyGateway.sdk = null;
  ...
  buildSettingsUI(wrapper) {
    ...
    this.settingsPanel.mount(scrollableContent, {
      onSave: async (settings) => {
        log.debug('Settings saved, refreshing providers...');
        this.updateContextCharLimit(settings);
        ...
        if (!this.isProviderConfigured(this.currentProvider)) {
          throw new Error(
            `${selectedProvider?.name || this.currentProvider} is not ready yet. Check your settings and try again.`,
          );
        }
  ...
  async switchProvider(providerKey) {
    log.debug('🔄 Switching to provider:', providerKey);
  ...
  showSettings() {
    log.debug('⚙️ Showing settings');
    this.showingSettings = true;
    this.updateUI();
  }

  hideSettings() {
    log.debug('⚙️ Hiding settings');
    this.showingSettings = false;
    this.updateUI();
  }
  ...
  async handleSendMessage(message) {
    try {
      ...
      log.debug('Current provider:', this.currentProvider, provider);
      ...
      const isOllama =
        settings?.endpoint?.includes('ollama') || settings?.endpoint?.includes('11434');
      ...
      log.debug('All context from pills:', allContext);
      log.debug('Context details:');
      allContext.forEach((ctx, i) => {
        log.debug(`  Context ${i}: ${ctx.title} - ${ctx.content?.length || 0} chars`);
      });

      // 🏷️ TAG CONTEXT EXPANSION
      const tagEnhancement = await tagContextExpander.enhanceConversationWithTags(
        message,
        allContext,
      );
      if (tagEnhancement) {
        log.debug(
          '🎯 Tag enhancement applied:',
          tagEnhancement.relatedTags.map((t) => `#${t.tag}`).join(', '),
        );
        ...
        if (tagEnhancement.additionalContext.length > 0) {
          const additionalFiles = tagEnhancement.additionalContext
            .map((note) => note.file)
            .join(', ');
          this.interface.addMessage({
            type: 'context',
      ...
      const completed = await this.streamProviderResponse(
        provider,
        message,
        allContext,
        tagEnhancement,
  ...
  async streamProviderResponse(provider, message, allContext, tagEnhancement) {
    log.debug(`🤖 Using ${provider.name} provider`);
    ...
    try {
      for await (const event of provider.sdk.stream({
        message,
        context: allContext,
        tagEnhancement,
        history: this.interface.getMessages?.() || [],
      })) {
        switch (event.type) {
          case 'start':
            log.debug(`🚀 ${provider.name} started, model:`, event.model);
            if (isClaudeAgentProvider) {
              this.updateClaudeCostDisplay(event);
            }
            break;
          ...
          case 'tool_start':
            log.debug('🔧 Tool started:', event.toolName);
            break;

          case 'tool_use':
            log.debug('🔧 Tool use:', event.toolName, event.toolInput);
            this.interface.addToolUse(event.id, event.toolName, event.toolInput);
            break;

          case 'tool_result':
            log.debug('✅ Tool result:', event.toolName, event.id);
            this.interface.updateToolResult(event.id, event.result);
            break;

          case 'task_created':
            log.debug('📋 Task created:', event.meta);
            this.interface.addTaskCreated(event.meta);
            this.interface.hideTyping();
            break;

          case 'assistant':
            this.applyAssistantEvent(event, streamingMessage, messageId, () => {
              if (!messageAdded) {
                this.interface.addMessage(streamingMessage);
                this.interface.hideTyping();
                messageAdded = true;
          ...
          case 'result':
            ...
            if (
              ...
              messageAdded &&
              event.text !== streamingMessage.content
            ) {
              streamingMessage.content = event.text;
              this.interface.updateMessage(messageId, streamingMessage.content);
          ...
          default:
            console.warn('Unknown provider event:', event);
            break;
        }
      }
      ...
      console.error(`${provider.name} error:`, providerError);
      finalizeOrHide();
      this.interface.addMessage({
        type: 'error',
        content: `${provider.name} error: ${providerError.message}`,
        timestamp: new Date(),
      });
      return false;
    }
  ...
  applyAssistantEvent(event, streamingMessage, messageId, ensureMessageAdded) {
  ...
  async getAllContext({ activeNote: providedActiveNote = undefined } = {}) {
    ...
    // Get active note
    log.debug('Getting all context...');
    const activeNote =
      providedActiveNote !== undefined ? providedActiveNote : await this.getActiveNoteContent();
    log.debug('Active note:', activeNote ? `Found: ${activeNote.title}` : 'None');
    ...
    log.debug('Mentioned notes:', mentionedNotes.length);
    ...
    log.debug('Total context notes:', contextNotes.length);
    return contextNotes;
  }
  ...
  async getCsvContext(path, title) {
    try {
      ...
      log.debug('Got rich CSV AI context for:', title);
      ...
      if (isPremiumError) {
        log.debug('CSV AI context requires premium, falling back to basic content');
  ...
  async getActiveNoteContent() {
    ...
    log.debug('Getting active note content...');

    const activeTab = getActiveTab();
    log.debug('Active tab:', activeTab);

    if (!activeTab) {
      log.debug('No active tab');
      return null;
    }
    ...
    if (isCsv) {
      log.debug('Active file is CSV, getting rich context...');
    ...
    if ((!content || content.length === 0) && filePath) {
      log.debug('No content from editor, trying to read from file:', filePath);
      try {
        content = await this.getNoteContent(filePath);
        log.debug('Got content from file:', content?.length || 0, 'chars');
      } catch (error) {
    ...
    log.debug('Got content from:', title, 'Length:', truncatedContent.length);
  ...
  // Cheap mode-class sync (no DOM rebuild). Safe to call on every show.
  applyModeClasses() {
    ...
    if (rightSidebar) {
      rightSidebar.classList.toggle('cli-mode', this.currentMode === 'cli');
    }
  }
  ...
  updateUI() {
    ...
    // Full rebuild of the message list — only do this when content/state has
    // actually changed, never merely to reveal an already-built panel.
  ...
  setupVaultListener() {
    ...
    if (window.windowContext) {
      this.vaultOpenedListener = async (vaultInfo) => {
        log.debug('EnhancedChatPanel: Vault opened event received:', vaultInfo);

        if (!vaultInfo?.path) {
          log.debug('EnhancedChatPanel: Ignoring vault-opened event without a vault path');
          return;
        }
        ...
        if (this.currentMode === 'cli' && this.cliContainer) {
          log.debug('EnhancedChatPanel: Resetting CLI for new vault:', vaultInfo.path);
  ...
  toggle() {
    try {
      log.debug(`🔄 Toggling chat panel visibility: ${this.isVisible} -> ${!this.isVisible}`);
      ...
      if (this.isVisible) {
        ...
        // The panel is built once at mount and stays built while hidden;
        // revealing it is purely CSS. Only sync mode classes — do NOT rebuild
        // the whole message list on every show (that scales with message count
        // and is the source of toggle lag).
        this.applyModeClasses();
        log.debug('✅ Chat panel shown');
      } else {
        rightSidebar.classList.remove('visible');
        // Remove inline width style to ensure CSS takes over
        rightSidebar.style.width = '';
        log.debug('✅ Chat panel hidden');
      }
      ...
      log.debug('💬 Chat panel toggled:', this.isVisible ? 'visible' : 'hidden');
    } catch (error) {
  ...
  async loadSavedProvider() {
    ...
    try {
      ...
      if (this.providers[providerKey]) {
        this.currentProvider = providerKey;
        log.debug('Loaded active provider from backend:', providerKey);
        ...
        log.debug('Unknown provider from backend:', activeProvider, '- defaulting to openai');
        this.currentProvider = 'openai';
        if (this.currentMode === 'botcky') {
          this.currentMode = 'chat';
          localStorage.setItem('gaimplan-chat-mode', 'chat');
  ...
  async exportChatSession(session = this.getActiveChatSession()) {
    ...
    log.debug('✅ Chat exported successfully to:', filePath);
  ...
  async destroy() {
    log.debug('🧹 Destroying EnhancedChatPanel');
