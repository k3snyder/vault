...
export class ChatInterface {
  ...
  updateContextIndicator() {
    ...
    this.currentContext.forEach((note) => {
      ...
      if (
        note.type === 'active' ||
        (activeNoteKey && noteKey === activeNoteKey) ||
        (note.path && seenPaths.has(note.path))
      ) {
        return;
      }
      if (note.path) {
  ...
  excludeActiveNoteFromContext(note) {
    ...
    if (note.path) {
      this.currentContext = this.currentContext.filter(
        (contextNote) => contextNote.path !== note.path,
      );
  ...
  appendMessageElement(message, previousNewest) {
    ...
    const previousDateKey = previousNewest
      ? this.getMessageDateKey(previousNewest.timestamp)
      : null;
  ...
  updateMessage(messageId, newContent) {
    ...
    if (messageIndex !== -1) {
      ...
      const messageEl = document.querySelector(
        `${this.getMessageSelector(messageId)} .message-content`,
      );
      if (messageEl) {
  ...
  finalizeStreamingMessage(messageId) {
    ...
    const messageEl = document.querySelector(
      `${this.getMessageSelector(messageId)} .streaming-cursor`,
    );
    if (messageEl) {
      messageEl.remove();
    }
  ...
  loadMessages(messages = null, options = {}) {
    try {
      ...
      if (Array.isArray(messages)) {
        this.messages = messages.map((msg) => this.normalizeStoredMessage(msg)).filter(Boolean);
      ...
      if (saved) {
        ...
        this.messages = Array.isArray(parsed)
          ? parsed.map((msg) => this.normalizeStoredMessage(msg)).filter(Boolean)
          : [];
        this.messages = this.getMessagesInDisplayOrder();
