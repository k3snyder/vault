...
import { getCurrentFile } from '../app/workspace.js';
...
export class ContextManager {
  ...
  addActiveNoteToContext() {
    ...
    if (currentFile) {
      console.log('📄 Adding active note to context:', currentFile);
  ...
  async searchNotes(query) {
    ...
    try {
      ...
      if (!query) {
        const allFiles = fileTree.files.filter(isSupportedFile).slice(0, 10);
