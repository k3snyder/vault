...
const log = createLogger('ClaudeAgentSDK');
...
export class ClaudeAgentSDK {
  constructor() {
    log.debug('Initializing ClaudeAgentSDK');
    this.settings = null;
    this.client = null;
  ...
  async initialize(settings) {
    log.debug('Initializing Claude Agent SDK...');
    ...
    try {
      ...
      if (hasAuthToken) {
        ...
        log.debug('Using OAuth token authentication (Bearer)');
      } else if (hasApiKey) {
        ...
        if (key.startsWith('sk-ant-oat')) {
          ...
          log.debug(
            '🔐 Detected OAuth token (sk-ant-oat...), using Bearer auth for Max subscription',
          );
        } else if (key.startsWith('sk-ant-')) {
          // Traditional API key
          effectiveApiKey = key;
          log.debug('🔑 Using API key authentication (X-Api-Key)');
        } else {
          // Unknown format - try as OAuth token
          effectiveAuthToken = key;
          log.debug('⚠️ Unknown token format, trying Bearer auth');
        }
      }
      ...
      this.settings = {
        ...
        maxToolCallsPerTurn: Math.min(
          settings.maxToolCallsPerTurn || MAX_TOOL_CALLS_PER_TURN,
      ...
      if (effectiveAuthToken) {
        ...
        log.debug('🔐 Anthropic client configured with authToken (Bearer auth)');
      } else {
        clientOptions.apiKey = effectiveApiKey;
        clientOptions.authToken = null;
        log.debug('🔑 Anthropic client configured with apiKey (X-Api-Key)');
      }

      log.debug('Client options:', {
      ...
      log.debug('Claude Agent SDK initialized with model:', this.currentModel);
      return true;
    } catch (error) {
  ...
  createToolDefinitions() {
    log.debug('Creating tool definitions...');
    ...
    this.tools = [
      ...
      },
      {
        ...
        input_schema: {
          ...
          properties: {
            path: {
              type: 'string',
              description: "Path to the note file relative to vault root (e.g., 'folder/note.md')",
            },
          },
          required: ['path'],
        },
      ...
      // CSV Editor Pro tools (Premium features)
      {
        ...
        description:
          'List all CSV files in the vault. Returns file information including path, name, size, and whether a schema exists.',
        input_schema: {
          type: 'object',
          properties: {},
        },
      },
      {
        ...
        description:
          'Get the schema for a CSV file. Returns column definitions with data types, semantic roles, and descriptions. Premium feature - creates schema if missing when createIfMissing is true.',
        input_schema: {
          type: 'object',
          properties: {
            path: {
              type: 'string',
              description: "Path to the CSV file relative to vault root (e.g., 'data/sales.csv')",
            },
            createIfMissing: {
              type: 'boolean',
              description: 'If true and no schema exists, infer and create one (requires premium)',
              default: false,
            },
          },
          required: ['path'],
      ...
      },
      {
        ...
        description:
          'Get AI-optimized context for a CSV file. Returns rich metadata including schema summary, column descriptions, sample data as markdown table, and relationship context. Premium feature - ideal for understanding CSV structure before analysis.',
        input_schema: {
          type: 'object',
          properties: {
            path: {
            ...
            maxSampleRows: {
              type: 'number',
              description: 'Maximum number of sample rows to include in context',
              default: 10,
            },
          },
          required: ['path'],
    ...
    log.debug('Created', this.tools.length, 'tool definitions');
  }

  // Tool Handlers - All path validation is done in Rust backend

  async handleSearchNotes(args) {
    log.debug('search_notes called:', args);
    try {
      ...
      log.debug('search_notes found', limitedResults.length, 'results');
  ...
  async handleGetNote(args) {
    log.debug('get_note called:', args);
    try {
      if (!args.path) {
      ...
      if (!result.success) {
        log.debug('get_note failed:', result.message);
        return JSON.stringify({ error: result.message, path: args.path });
      }

      log.debug('get_note read', result.length, 'characters');
  ...
  async handleGetCurrentNote() {
    log.debug('get_current_note called');
    try {
      const activeTabManager = getActiveTabManager();
      if (!activeTabManager) {
  ...
  async handleListTags(args) {
    log.debug('list_tags called:', args);
    try {
      // Use new secure Rust command
      const tags = await invoke('agent_list_tags', { limit: args?.limit || 50 });
      log.debug('list_tags found', tags.length, 'tags');
  ...
  async handleNotesByTag(args) {
    log.debug('notes_by_tag called:', args);
    try {
      if (!args.tag) {
  ...
  async handleSemanticSearch(args) {
    log.debug('semantic_search called:', args);
    try {
      if (!args.query) {
        return JSON.stringify({ error: 'Query is required' });
      }
      // Use new secure Rust command
      const results = await invoke('agent_semantic_search', {
        query: args.query,
        limit: args.limit || 10,
      });
      log.debug('semantic_search found', results.length, 'results');
  ...
  async handleWriteNote(args) {
    log.debug('write_note called:', args);
    try {
      ...
      if (!result.success) {
        log.debug('write_note failed:', result.message);
        return JSON.stringify({ error: result.message, path: args.path });
      }

      log.debug('write_note created:', args.path);
      return JSON.stringify({
        success: true,
        path: result.path,
  ...
  async handleUpdateNote(args) {
    log.debug('update_note called:', args);
    try {
      if (!args.path || !args.content) {
        return JSON.stringify({ error: 'Path and content are required' });
      }
      ...
      if (!result.success) {
        log.debug('update_note failed:', result.message);
        return JSON.stringify({ error: result.message, path: args.path });
      }

      log.debug('update_note updated:', args.path);
      return JSON.stringify({
        success: true,
        path: result.path,
        message: result.message,
  ...
  async handleAppendToNote(args) {
    log.debug('append_to_note called:', args);
    try {
      if (!args.path || !args.content) {
        return JSON.stringify({ error: 'Path and content are required' });
      }
      ...
      if (!result.success) {
        log.debug('append_to_note failed:', result.message);
        return JSON.stringify({ error: result.message, path: args.path });
      }

      log.debug('append_to_note appended to:', args.path);
      return JSON.stringify({
        success: true,
        path: result.path,
        message: result.message,
      ...
      return JSON.stringify({
        error: error.message || 'Failed to append to note',
        path: args.path,
      });
    }
  }
  ...
  async handleListCsvFiles() {
    log.debug('list_csv_files called');
    try {
      const files = await invoke('list_csv_files');
      log.debug('list_csv_files found', files.length, 'CSV files');
  ...
  async handleGetCsvSchema(args) {
    log.debug('get_csv_schema called:', args);
    try {
      if (!args.path) {
        return JSON.stringify({ error: 'Path is required' });
      }
      ...
      log.debug('get_csv_schema loaded schema for:', args.path);
  ...
  async handleGetCsvContext(args) {
    log.debug('get_csv_context called:', args);
    try {
      if (!args.path) {
        return JSON.stringify({ error: 'Path is required' });
      }
      ...
      log.debug('get_csv_context generated context for:', args.path);
  ...
  async executeTool(toolName, toolInput) {
    log.debug('Executing tool:', toolName, toolInput);
  ...
  getFilteredTools() {
    ...
    log.debug('Allowed tools:', allowed);
  ...
  updateSettings(settings) {
    if (settings) {
      ...
      // Enforce limits
      this.settings.maxTurns = Math.min(
        this.settings.maxTurns || MAX_TURNS_DEFAULT,
        MAX_TURNS_ABSOLUTE,
      );
  ...
  async *chat(message, context = [], options = {}) {
    ...
    log.debug('Chat called with:', message);
    log.debug('With context:', context.length, 'notes');
...
Be concise and helpful. When summarizing content, extract the key points and present them clearly.`;
      ...
      log.debug('Starting SDK query with model:', this.currentModel);
      log.debug('Limits: maxTurns=%d, maxToolCallsPerTurn=%d', maxTurns, maxToolCallsPerTurn);
      ...
      while (turnCount < maxTurns) {
        ...
        if (!hasToolUse) {
          ...
          log.debug('Chat query completed (no tool use)');
        ...
        if (toolUseBlocks.length > maxToolCallsPerTurn) {
          console.warn(
            `Tool call limit exceeded: ${toolUseBlocks.length} > ${maxToolCallsPerTurn}`,
          );
          yield {
            type: 'warning',
        ...
        // Handle tool use
        log.debug('Processing', toolUseBlocks.length, 'tool calls');
