...
export class OpenAISDK {
  ...
  async formatMessages(userMessage, context = [], tagContext = null) {
    ...
    const defaultPrompt =
      'You are a helpful AI assistant integrated into a note-taking app called Vault. You help users with their notes, writing, research, and questions. Always provide helpful, accurate, and relevant responses.';
  ...
  truncateContext(context, maxContextTokens = 2000) {
    ...
    console.log(
      `Context truncated: ${context.length} -> ${truncatedContext.length} notes, ~${currentTokens} tokens`,
    );
