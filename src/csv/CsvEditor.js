...
const log = createLogger('CsvEditor');
...
export class CsvEditor {
  ...
  constructor(filePath, tabManager, paneId) {
    log.debug(`Creating CSV editor for: ${filePath}`);
   ...
   */
  async mount() {
    log.debug('Mounting CSV editor for:', this.filePath);
    ...
    try {
      ...
      log.debug('CSV editor mounted successfully');
    } catch (error) {
   ...
   */
  async loadData() {
    log.debug('Loading CSV data from:', this.filePath);
    ...
    try {
      ...
      log.debug('CSV Editor premium status:', this.state.isPremium);
      ...
      if (this.state.isPremium) {
        ...
        if (schema) {
          ...
          log.debug('CSV schema loaded');
        } else {
          log.debug('No schema found for this file (can be created)');
  ...
  render() {
    ...
    if (this.shouldUseVirtualScroll()) {
      log.debug(`Using virtual scrolling for ${this.state.workingRows.length} rows`);
  ...
  renderToolbar() {
    ...
    const displayCount =
      rowCount === totalRows ? `${rowCount} rows` : `${rowCount} of ${totalRows} rows`;
  ...
  renderVirtualTable() {
    ...
    log.debug('📊 Virtual table column widths:', columnWidths);
    log.debug('📊 Headers:', data.headers);
  ...
  updateVisibleRows() {
    ...
    // Top spacer reserves the height of every row above the rendered window so
    // the scroll container reports the full scrollable height and the visible
    // rows sit at the correct vertical offset (single-table virtual scroll).
    const colCount = this.state.data.headers.length + 1;
    const topPad = start * this.ROW_HEIGHT;
    if (topPad > 0) {
      fragment.appendChild(this.createVirtualSpacerRow(topPad, colCount));
    }
    ...
    // Bottom spacer reserves the height of every row below the rendered window.
    const bottomPad = (this.state.workingRows.length - end) * this.ROW_HEIGHT;
    if (bottomPad > 0) {
      fragment.appendChild(this.createVirtualSpacerRow(bottomPad, colCount));
    }
    ...
    if (editingCell && editorDom) {
      ...
      if (row >= start && row < end) {
        const cellElement = this.virtualTbody.querySelector(
  ...
  /**
   * Build a zero-content row that reserves vertical space for the off-screen
   * rows of a virtual-scrolled table. Keeping the spacer inside the tbody
   * preserves the single-table layout (and its sticky header / colgroup column
   * alignment) while giving the scroll container its full height.
   * @param {number} height - Height to reserve, in pixels
   * @param {number} colCount - Columns to span (row number + data columns)
   * @returns {HTMLTableRowElement}
   */
  createVirtualSpacerRow(height, colCount) {
    const tr = document.createElement('tr');
    tr.className = 'csv-virtual-spacer-row';
  ...
  setupVirtualScrollHandlers() {
    ...
    this.virtualScrollContainer.addEventListener('scroll', this.boundScrollHandler, {
      passive: true,
    });
  ...
  toggleSchemaSidebar() {
    ...
    log.debug('Schema sidebar toggled:', this.state.schemaSidebarOpen);
  }
   ...
   */
  async inferSchema() {
    log.debug('Inferring schema for:', this.filePath);
    log.debug('Frontend isPremium state:', this.state.isPremium);

    // Debug: Check entitlement status before calling backend
    if (this.entitlementManager) {
      const status = this.entitlementManager.getStatus();
      log.debug('EntitlementManager status:', JSON.stringify(status));
    }
    ...
    try {
      ...
      log.debug('Schema inferred (not yet saved):', schema);
      ...
      const isPremiumError =
        error.code === 'premiumRequired' || (error.message && error.message.includes('Premium'));
  ...
  handleDescriptionChange(e) {
    ...
    if (column.description !== newDescription) {
      ...
      log.debug(`Updated description for column ${colIndex}:`, newDescription);
  ...
  handleRoleChange(e) {
    ...
    this.updateSaveButtonState();
    this.updateDirtyIndicator();
    log.debug(`Updated role for column ${colIndex}:`, newRole);
  ...
  debouncedSaveSchema() {
    ...
    if (this.state.schemaUnsaved) {
      log.debug('Schema not yet saved - skipping auto-save');
      return;
    }
  ...
  async saveSchemaToFile() {
    if (!this.state.schema || this.state.schema.readOnly) {
      log.debug('Cannot save schema: no schema or read-only');
      return;
    }

    log.debug('Saving schema to file for:', this.filePath);
    ...
    try {
      ...
      log.debug('Schema saved to file successfully');
  ...
  async saveSchema() {
    ...
    log.debug('Saving schema for:', this.filePath);
    ...
    try {
      ...
      log.debug('Schema saved successfully');
  ...
  hasMultiCellSelection() {
    ...
    return (
      this.state.selectionStart.row !== this.state.selectionEnd.row ||
  ...
  clearSelectionHighlight() {
    ...
    const selectedCells = cellContainer.querySelectorAll(
      '.csv-cell-selected, .csv-cell-in-selection',
    );
  ...
  startEditing(row, col) {
    ...
    log.debug('Start editing cell:', { row, col });
    ...
    log.debug('Cell editing started:', { row, col, value: currentValue });
  }
    ...
    return [
      ...
      EditorView.updateListener.of((update) => {
        if (update.docChanged && this.state.editingCell) {
          ...
          log.debug('Cell content changed');
        }
      }),
    ];
  }
  ...
  finishEditing() {
    ...
    log.debug('Finishing edit:', { row, col, oldValue, newValue });
  ...
  cancelEditing() {
    ...
    log.debug('Canceling edit:', { row, col, restoredValue: originalValue });
  ...
  clearCell(row, col) {
    ...
    if (currentValue !== '') {
      ...
      log.debug('Cell cleared:', { row, col });
    }
  }
  ...
  async pasteFromClipboard() {
    const { selectedCell } = this.state;
    if (!selectedCell) {
      log.debug('No cell selected for paste');
      return;
    }
    ...
    for (let r = 0; r < pasteRows && startRow + r < numRows; r++) {
      for (let c = 0; c < (pasteData[r]?.length || 0) && startCol + c < numCols; c++) {
        ...
        // Update display
        const cellElement = cellContainer?.querySelector(
    ...
    log.debug('Pasted from clipboard:', {
      startRow,
      startCol,
  ...
  addRow(position = 'bottom') {
    log.debug('Add row triggered:', position);
    ...
    log.debug('Row added at index:', insertIndex);
  }
  ...
  async addColumn(position = 'after') {
    log.debug('Add column triggered:', position);
    ...
    if (columnName === null) {
      log.debug('Add column cancelled by user');
      return;
    }
    ...
    log.debug('Column added:', trimmedName, 'at index:', insertIndex);
  }
   ...
   */
  deleteRow() {
    log.debug('Delete row triggered');
    ...
    log.debug('Row deleted at index:', rowIndex);
  }
   ...
   */
  deleteColumn() {
    log.debug('Delete column triggered');
    ...
    log.debug('Column deleted:', deletedHeader, 'at index:', colIndex);
  }
  ...
  async save() {
    ...
    try {
      ...
      log.debug('CSV data saved successfully');
   ...
   */
  unmount() {
    log.debug('Unmounting CSV editor');
    ...
    log.debug('CSV editor unmounted');
  }
  ...
  async openRelationshipEditor(editIndex = null) {
    log.debug('Opening relationship editor, editIndex:', editIndex);

    // Get existing relationship if editing
    const existingRelationship =
      editIndex !== null ? this.state.schema?.relationships?.[editIndex] : null;
    ...
    if (existingRelationship?.foreignFile) {
      await this.loadForeignColumns(
        existingRelationship.foreignFile,
        foreignColumnSelect,
        existingRelationship.foreignColumn,
      );
    }
  ...
  async saveRelationship() {
    ...
    log.debug('Saving relationship:', relationship);
  ...
  async deleteRelationship(index) {
    ...
    log.debug('Deleting relationship at index:', index);
  ...
  setupRelationshipHandlers() {
    ...
    const addBtns = this.schemaSidebar.querySelectorAll(
      '.csv-add-relationship-btn, .csv-add-relationship-btn-empty',
    );
    addBtns.forEach((btn) => {
  ...
  async openAiContextModal() {
    log.debug('Opening AI context modal for:', this.filePath);
    ...
    try {
      ...
      log.debug('AI context received:', aiContext);
  ...
  parseSampleDataToJson(sampleData, columns) {
    try {
      ...
      const dataRows = lines.filter((line) => {
        ...
        return trimmed.startsWith('|') && !trimmed.match(/^\|[-:\s|]+\|$/) && line !== headerLine;
      });
  ...
  renderAiContextError(modal, error) {
    ...
    // Get user-friendly error info
    const errorInfo = getUserFriendlyMessage(CsvErrorType.SCHEMA_ERROR, error);
  ...
  async startFreeTrial() {
    try {
      ...
      this.state.isPremium = this.entitlementManager.isPremiumEnabled();
      log.debug('Trial started, premium status:', this.state.isPremium);
  ...
  async exportAsCsv() {
    try {
      ...
      if (savePath) {
        ...
        log.debug('CSV exported to:', savePath);
    ...
    try {
      ...
      if (!range) {
        ...
        if (this.state.selectedCell) {
          ...
          if (savePath) {
            await invoke('export_to_file', { path: savePath, content: csvContent });
            log.debug('Selection exported to:', savePath);
            this.showToast('Exported selection');
          }
        } else {
          log.debug('No selection to export');
        }
        return;
      }
  ...
  async exportAsJson() {
    try {
      ...
      if (savePath) {
        ...
        log.debug('JSON exported to:', savePath);
  ...
  async copyAsJson() {
    try {
      ...
      log.debug('Copied JSON to clipboard');
  ...
  async copyAsMarkdown() {
    try {
      ...
      log.debug('Copied markdown table to clipboard');
  ...
  async importDroppedFile(file) {
    try {
      log.debug('Importing dropped file:', file.name);
      ...
      log.debug('Detected delimiter:', delimiter === '\t' ? 'TAB' : delimiter);
