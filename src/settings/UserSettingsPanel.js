...
import { getActiveTab, getActiveTextEditor, getPaneManager } from '../app/workspace.js';
import { toggleStatusBar } from '../app/chrome.js';
import {
...
export class UserSettingsPanel {
  constructor() {
    this.state = {
      ...
      network: {
        ...
        allowPrivateNetworkImages: false,
      },
      isDirty: false,
      isSaving: false,
      isLoading: true,
  ...
  async loadSettings() {
    try {
      ...
      this.state.editor = {
        ...
        lineNumbers:
          settings.editor.line_numbers !== undefined
            ? settings.editor.line_numbers
            : this.state.editor.lineNumbers,
        lineWrapping:
          settings.editor.line_wrapping !== undefined
            ? settings.editor.line_wrapping
            : this.state.editor.lineWrapping,
        showStatusBar:
          settings.editor.show_status_bar !== undefined
            ? settings.editor.show_status_bar
            : this.state.editor.showStatusBar,
        wysiwygMode:
          settings.editor.wysiwyg_mode !== undefined
            ? settings.editor.wysiwyg_mode
            : this.state.editor.wysiwygMode,
        themeOverrides: normalizeThemeOverrides(
          settings.editor.theme_overrides || settings.editor.themeOverrides,
        ),
      };
      this.committedEditorState = this.cloneEditorState(this.state.editor);
      this.state.files = {
        ...this.state.files,
        imageLocation: normalizeImageLocation(
          settings.files.image_location || this.state.files.imageLocation,
        ),
        imageNamingPattern:
          settings.files.image_naming_pattern || this.state.files.imageNamingPattern,
        dailyNotesFolder: settings.files.daily_notes_folder || this.state.files.dailyNotesFolder,
      };
      this.state.network = {
        ...this.state.network,
        allowHttpImages: settings.network?.allow_http_images ?? this.state.network.allowHttpImages,
        allowPrivateNetworkImages:
          settings.network?.allow_private_network_images ??
          this.state.network.allowPrivateNetworkImages,
      };
  ...
  async saveSettings() {
    ...
    try {
      ...
      const settings = {
        ...
        network: {
          allow_http_images: this.state.network.allowHttpImages,
          allow_private_network_images: this.state.network.allowPrivateNetworkImages,
        },
      };
  ...
  async resetSection(section) {
    ...
    try {
      ...
      this.state.network = {
        ...this.state.network,
        allowHttpImages: settings.network?.allow_http_images ?? false,
        allowPrivateNetworkImages: settings.network?.allow_private_network_images ?? false,
      };
      this.state.isDirty = false;
  ...
  updateFileSetting(key, value) {
    this.state.files[key] = key === 'imageLocation' ? normalizeImageLocation(value) : value;
    this.state.isDirty = true;
    this.render();
  }

  updateNetworkSetting(key, value) {
    this.state.network[key] = value;
    this.state.isDirty = true;
    this.render();
  }
  ...
  previewChanges() {
    ...
    this.previewTimeout = setTimeout(() => {
      ...
      if (window.themeManager) {
        ...
        // Apply WYSIWYG mode
        const wysiwygEditor = getActiveTextEditor();
        if (typeof wysiwygEditor?.setWysiwygMode === 'function') {
          wysiwygEditor.setWysiwygMode(this.state.editor.wysiwygMode);
        }
  ...
  handleContainerChange(event) {
    ...
    switch (target.dataset.action) {
      ...
      case 'update-file-setting':
        ...
        break;
      case 'update-network-setting':
        this.updateNetworkSetting(target.dataset.setting, target.checked);
        break;
    }
  }
  ...
  applyEditorPreview(editorState) {
    ...
    const wysiwygEditor = getActiveTextEditor();
    if (typeof wysiwygEditor?.setWysiwygMode === 'function') {
      wysiwygEditor.setWysiwygMode(editorState.wysiwygMode);
    }
                    ...
                    <div class="settings-section" style="display: ${this.state.activeTab === 'editor' ? 'block' : 'none'}">
                        ...
                        <div class="settings-group">
                            ...
                            <div class="form-group checkbox-group">
                                <label>
                                    <input type="checkbox"
                                           ${this.state.network.allowHttpImages ? 'checked' : ''}
                                           data-action="update-network-setting"
                                           data-setting="allowHttpImages">
                                    Allow HTTP image URLs
                                </label>
                                <p class="form-help">HTTPS image URLs are preferred. Enable only for vaults that intentionally embed plain HTTP images.</p>
                            </div>

                            <div class="form-group checkbox-group">
                                <label>
                                    <input type="checkbox"
                                           ${this.state.network.allowPrivateNetworkImages ? 'checked' : ''}
                                           data-action="update-network-setting"
                                           data-setting="allowPrivateNetworkImages">
                                    Allow private-network image URLs
                                </label>
                                <p class="form-help">Allows embedded images from localhost, RFC1918, link-local, ULA, and CGNAT addresses for this vault.</p>
                            </div>
                        </div>
                    </div>
                </div>
