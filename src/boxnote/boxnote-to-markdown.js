...
import { openFile } from '../app/file-open.js';
...
function looksLikeMarkdownAuthoredText(value = '') {
  ...
  if (
    /^(?:#{1,6}\s+\S|[-*_](?:\s*[-*_]){2,}\s*|>\s+|(?:[-+*]|\d+\.)\s+|- \[[ xX]\]\s+)/mu.test(text)
  ) {
    return true;
  }
...
function tableToMarkdown(node) {
  ...
  const renderedRows = rows.map((row) => {
    const cells = (row.content || []).map((cell) => {
      const cellText = normalizeInlineWhitespace(inlineNodesToMarkdown(cell.content || [])).replace(
        /\|/gu,
        '\\|',
      );
      return cellText || ' ';
...
function listItemToMarkdown(node, options) {
  const { indent = '', marker = '- ', orderedIndex = 1, ordered = false } = options;
  ...
  const normalizedInlineContent =
    !ordered && marker === '- ' ? unescapeLeadingTaskMarker(inlineContent) : inlineContent;
  ...
  switch (node.type) {
    ...
    case 'check_list_item':
      return listItemToMarkdown(node, {
        ...
        marker:
          node.type === 'check_list_item' ? (node.attrs?.checked ? '- [x] ' : '- [ ] ') : '- ',
      });
    case 'table':
...
async function findAvailableMarkdownPath(originalFilePath) {
  ...
  const candidates = [`${basePath}.md`, `${basePath} (from Box Note).md`];
...
export async function convertBoxNoteFile(filePath) {
  ...
  showNotification(`Converted to ${outputPath.split('/').pop()}`, 'success');
