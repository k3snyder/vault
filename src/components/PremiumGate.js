  ...
  render() {
    ...
    if (status.status === 'unlicensed') {
      description.textContent =
        'Unlock semantic search and cognitive memory features with a free 30-day trial.';

      const trialButton = this.createButton('Start Free Trial', 'primary', () =>
        this.handleStartTrial(),
      );
      const purchaseButton = this.createButton('Purchase License', 'secondary', () =>
        this.handlePurchase(),
      );
      ...
      description.textContent =
        'Your trial has expired. Purchase a license to continue using premium features.';

      const purchaseButton = this.createButton('Purchase License', 'primary', () =>
        this.handlePurchase(),
      );
      buttonContainer.appendChild(purchaseButton);
