...
import { closeCurrentVault } from '../app/vault-lifecycle.js';
  ...
  render() {
    ...
    const iconMarkup = this.options.showIcon
      ? `
        ...
        `
      : '';
    const vaultName = this.currentVault
      ? this.escapeHtml(this.currentVault.name)
      : this.escapeHtml(this.options.emptyLabel);
