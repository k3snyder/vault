...
export class IntelligencePanel {
  ...
  hasEnrichments() {
    ...
    return this.result.enrichments.some(
      (e) =>
  ...
  createStatsSection() {
    ...
    const statItems = [
      ...
      {
        label: 'Vision',
        value: stats.visionTimeMs ? `${(stats.visionTimeMs / 1000).toFixed(1)}s` : 'N/A',
      },
      {
        label: 'Summarization',
        value: stats.summarizationTimeMs
          ? `${(stats.summarizationTimeMs / 1000).toFixed(1)}s`
          : 'N/A',
      },
    ];
