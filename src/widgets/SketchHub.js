...
import { handleFileClick } from '../app/file-open.js';
import { expandFolder, refreshFileTree } from '../app/file-tree-view.js';

const log = createLogger('SketchHub');
...
export class SketchHub {
  ...
  render() {
    ...
    this.countElement.textContent =
      this.sketches.length === 1 ? '1 sketch' : `${this.sketches.length} sketches`;
  ...
  createSketchRow(sketch) {
    ...
    row.addEventListener('click', async () => {
      try {
        await handleFileClick(sketch.path, false);
        ...
        try {
          toast.error('Failed to open sketch', 2000);
        } catch (error) {
  ...
  async handleCreateSketch() {
    try {
      const rawName =
        typeof this.options.requestSketchName === 'function'
      ...
      try {
        toast.error('Failed to create sketch', 2000);
      } catch (error) {
        log.swallow('suppressed error', error);
      }
    }
