...
function buildImageNotice({ icon, title, detail, hint }) {
  ...
  iconNode.style.cssText = 'font-size: 24px; margin-bottom: 8px;';
  ...
  if (detail) {
    ...
    detailNode.style.cssText = 'font-size: 11px; opacity: 0.8;';
  ...
  if (hint) {
    ...
    hintNode.style.cssText = 'font-size: 11px; opacity: 0.6; margin-top: 8px;';
...
class ImageWidget extends WidgetType {
  ...
  toDOM() {
    ...
    invoke('fetch_image_as_base64', {
      url: this.url,
      vaultPath: window.currentVaultPath ?? null,
    })
      .then((base64Data) => {
        img.src = base64Data;
        wrapper.innerHTML = '';
      ...
      .catch((error) => {
        ...
        wrapper.appendChild(
          buildImageNotice({
            ...
            detail: this.url.length > 50 ? `${this.url.substring(0, 50)}...` : this.url,
            hint: String(error),
          }),
        );
      });
  ...
  toDOM() {
    ...
    if (!this.isLikelyImage) {
      ...
      wrapper.replaceChildren(
        buildImageNotice({
          icon: 'File',
          title: 'Cannot embed non-image file',
          detail: this.filename,
          hint: `Use [[${this.filename}]] for note links instead`,
        }),
      );
      return wrapper;
    }
