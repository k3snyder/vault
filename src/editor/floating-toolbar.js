...
function findSyntaxMarkRange(view, marker) {
  ...
  tree.iterate({
    ...
    enter: (node) => {
      ...
      if (
        !bestMatch ||
        candidate.closeTo - candidate.openFrom < bestMatch.closeTo - bestMatch.openFrom
      ) {
        bestMatch = candidate;
      }
    },
...
function findSingleStructuredLineContentRange(view) {
  ...
  const match = startLine.text.match(
    /^(\s*(?:#{1,6}\s+|- \[[ xX]\]\s+|[-*+]\s+|\d+\.\s+|>\s+))(.*)$/,
  );
  if (!match || !match[2]) {
    return null;
  }
      ...
      buttons.forEach((btn) => {
        ...
        if (isActive) {
          ...
          btn.style.backgroundColor =
            buttonConfig.id === 'highlight' ? 'rgba(255, 235, 59, 0.3)' : 'transparent';
...
export function floatingToolbarExtension() {
  return [floatingToolbarPlugin, floatingToolbarKeymap, floatingToolbarStyles];
}
