          ...
          enter: (node) => {
            ...
            if (STYLE_DECORATIONS[nodeName]) {
              ...
              if (nodeName.startsWith('ATXHeading')) {
                if (!isEditing) {
                  const { from: contentFrom, to: contentTo } = getHeadingContentRange(
                    node.node,
                    doc,
                  );
...
const wysiwygStyles = EditorView.theme({
  ...
  '.cm-wysiwyg-code': {
    fontFamily:
      'var(--font-mono, SFMono-Regular, Consolas, "Liberation Mono", Menlo, monospace) !important',
...
export function wysiwygExtension(enabled = true) {
  return [wysiwygEnabled.init(() => enabled), wysiwygPlugin, wysiwygAtomicRanges, wysiwygStyles];
}
...
export { STYLE_DECORATIONS, SYNTAX_MARKERS, wysiwygPlugin, wysiwygStyles, wysiwygAtomicRanges };
