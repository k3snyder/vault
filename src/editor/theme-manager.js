...
import {
  applyThemeOverrides,
  getThemeOverrideColor,
  normalizeThemeOverrides,
} from '../tokens/theme-overrides.js';
import { getPaneManager } from '../app/workspace.js';
...
export class ThemeManager {
  ...
  applyTheme(themeName, themeOverrides = this.themeOverrides) {
    ...
    if (themeName === 'light' || themeName === 'default') {
      ...
      this.updateWindowBackground(
        getThemeOverrideColor(themeName, this.themeOverrides, 'background') || '#ffffff',
        'light',
      );
      ...
      this.updateWindowBackground(
        getThemeOverrideColor(themeName, this.themeOverrides, 'background') || '#12110F',
        'dark',
      );
    } else {
    ...
    if (this.editor && this.editor.view) {
      this.editor.view.dispatch({
        effects: this.editor.themeCompartment.reconfigure(this.editor.createTheme(theme.type)),
      });
  ...
  setFontSize(size) {
    ...
    if (this.editor?.view && this.editor.fontSizeCompartment && this.editor.createFontSizeTheme) {
      this.editor.view.dispatch({
        effects: this.editor.fontSizeCompartment.reconfigure(this.editor.createFontSizeTheme(size)),
      });
    }
