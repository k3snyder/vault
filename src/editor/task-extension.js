...
import { getActiveTab } from '../app/workspace.js';

const log = createLogger('task-extension');
...
async function scheduleEditorSync(filePath, lineNumber) {
  ...
  window.__taskSyncTimers.set(
    key,
    setTimeout(async () => {
      try {
        const updated = await invoke('read_file_content', { filePath });
        const activeEditor = getActiveTab()?.editor;
        ...
        if (activeEditor && typeof activeEditor.setContent === 'function' && isActive) {
          ...
          if (typeof activeEditor.save === 'function') {
            try {
              await activeEditor.save();
            } catch (error) {
              log.swallow('suppressed error', error);
            }
          }
        }
      } catch (e) {
...
class CheckboxWidget extends WidgetType {
  ...
  async handleClick() {
    ...
    const relPath = this.view.state.field(window.currentFilePath) || getActiveTab()?.filePath;
    const filePath =
      relPath && (relPath.startsWith('/') || relPath.includes(':'))
        ? relPath
        : window.currentVaultPath && relPath
          ? `${window.currentVaultPath}/${relPath}`
          : relPath;
    ...
    try {
      ...
      try {
        ...
        if (activeEditor && typeof activeEditor.setContent === 'function') {
          ...
          const targetLine = doc
            ? Math.min(this.taskInfo.lineNumber, doc.lines)
            : this.taskInfo.lineNumber;
          const anchor = doc ? doc.line(targetLine).from : 0;
          activeEditor.setContent(updated, true, activeEditor.currentFile, true);
          if (activeEditor.view) activeEditor.view.dispatch({ selection: { anchor } });
          if (typeof activeEditor.save === 'function') {
      ...
      try {
        toast.success(newStatus === 'done' ? 'Task marked done' : 'Task updated', 1200);
      } catch (error) {
        log.swallow('suppressed error', error);
      }
    } catch (error) {
      ...
      try {
        toast.error('Failed to toggle task', 2000);
      } catch (error) {
        log.swallow('suppressed error', error);
      }
    }
...
class PropertyChipWidget extends WidgetType {
  ...
  toDOM() {
    ...
    if (
      ...
      this.value.toLowerCase().startsWith('project/')
    ) {
  ...
  eq(other) {
    return (
      other instanceof PropertyChipWidget && other.type === this.type && other.value === this.value
    );
  }
...
function createTaskDecorations(view) {
  ...
  for (const [lineNumber, taskInfo] of taskState) {
    ...
    if (
      editMode.has(lineNumber) ||
      isEmptyTaskContent ||
      window.__taskStickyEditLines.has(lineNumber)
    ) {
    ...
    if (taskInfo.id) {
      const tidRegex = /<!--\s*tid:\s*([a-f0-9-]+)\s*-->/gi;
      let m;
...
export const taskExtension = ViewPlugin.fromClass(
  class {
    ...
    update(update) {
      ...
      const hasRelevantEffect = update.transactions.some(
        (tr) =>
          tr.effects &&
          tr.effects.some((e) => e.is(updateTaskEffect) || e.is(toggleEditModeEffect)),
      );
          ...
          try {
            ...
            if (prevLine !== currLine) {
              ...
              if (wasEdit) {
                ...
                try {
                  window.__taskStickyEditLines.delete(prevLine);
                } catch (error) {
                  log.swallow('suppressed error', error);
                }
    ...
    async _ensureTaskUUIDs(view) {
      const filePath = view.state.field(window.currentFilePath) || getActiveTab()?.filePath;
...
const taskClickHandler = EditorView.domEventHandlers({
  mousedown(e, view) {
    ...
    if (!editMode.has(lineNumber)) {
      ...
      try {
        window.__taskStickyEditLines.delete(lineNumber);
      } catch (error) {
        log.swallow('suppressed error', error);
      }
    }
    view.dispatch({
      effects: toggleEditModeEffect.of(lineNumber),
...
function exitEditModeViaKeymap(view) {
  ...
  try {
    window.__taskStickyEditLines.delete(target);
  } catch (error) {
    log.swallow('suppressed error', error);
  }
...
export function taskExtensionConfig() {
  return [
    ...
    taskClickHandler,
    taskKeyHandler,
    keymap.of([{ key: 'Escape', run: exitEditModeViaKeymap }]),
  ];
}
...
async function ensureUuidForLine(view, lineNumber) {
  try {
    ...
    if (result && result.uuid) {
      ...
      view.dispatch({
        effects: updateTaskEffect.of({ lineNumber, taskInfo: { id: result.uuid } }),
      });
