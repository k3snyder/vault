...
function getRememberedTableColumnWidths(cacheKey) {
  ...
  if (Array.isArray(storedWidths)) {
    const normalizedWidths = storedWidths.map((width) =>
      Math.max(TABLE_MIN_COLUMN_WIDTH, Math.round(width)),
    );
...
class TableWidget extends WidgetType {
  ...
  captureColumnWidths(table, colgroup) {
    return Array.from(colgroup.children).map((col, columnIndex) => {
      ...
      const measuredWidth =
        rectWidth || cell?.offsetWidth || cell?.scrollWidth || TABLE_MIN_COLUMN_WIDTH;
...
class CodeBlockWidget extends WidgetType {
  ...
  toDOM() {
    ...
    const preserveNativeSelection = (e) => {
      if (e.target.closest?.('.cm-code-copy-btn')) {
        return;
      }
    ...
    // Prevent pane/editor click handlers from reclaiming focus during text selection.
    pre.addEventListener('mousedown', (e) => {
      preserveNativeSelection(e);
      pre.focus();
    });
    pre.addEventListener('mouseup', preserveNativeSelection);
    pre.addEventListener('click', preserveNativeSelection);

    // Preserve native copy behavior for partial code selections inside the widget.
  ...
  highlightJavaScript(content) {
    ...
    content = content.replace(
      /\b(const|let|var|function|if|else|for|while|return|class|extends|import|export|async|await|try|catch|finally|throw|new|this|super|static|get|set|true|false|null|undefined)\b/g,
    ...
    content = content.replace(
      /(['"`])((?:\\.|(?!\1)[^\\])*?)\1/g,
      '<span class="cm-string">$1$2$1</span>',
    );
  ...
  highlightPython(content) {
    // Keywords
    content = content.replace(
      /\b(def|class|if|elif|else|for|while|return|import|from|as|try|except|finally|raise|with|pass|break|continue|lambda|yield|global|nonlocal|assert|del|True|False|None|and|or|not|is|in)\b/g,
      '<span class="cm-keyword">$1</span>',
    );
  ...
  highlightHTML(content) {
    ...
    content = content.replace(
      /(\w+)=(['"])(.*?)\2/g,
      '<span class="cm-variable">$1</span>=<span class="cm-string">$2$3$2</span>',
    );
  ...
  highlightCSS(content) {
    ...
    content = content.replace(
      /(\w+):\s*([^;]+);/g,
      '<span class="cm-variable">$1</span>: <span class="cm-string">$2</span>;',
    );
...
const blockWidgetField = StateField.define({
  ...
  update(decorations, tr) {
    ...
    // Block widgets are selection-agnostic in current WYSIWYG mode.
    // Avoid rebuilding them on cursor moves so native widget selections survive.
    if (tr.docChanged) {
...
function computeBlockDecorations(state) {
  ...
  for (let i = 1; i <= doc.lines; i++) {
    ...
    const tableResult = processTableForStateField(doc, builder, line);
    if (tableResult) {
    ...
    const codeBlockResult = processCodeBlockForStateField(doc, builder, line);
    if (codeBlockResult) {
...
function processTableForStateField(doc, builder, line) {
...
function processCodeBlockForStateField(doc, builder, line) {
  const text = line.text.trim();
...
export const inlineFormattingExtension = Prec.highest(
  ViewPlugin.fromClass(
    class {
      ...
      processLineFormatting(view, decorations, atomicDecorations, line, isLineActive) {
        ...
        const headingInfo = this.processHeadingFormatting(
          decorations,
          atomicDecorations,
          text,
          lineStart,
        ...
        const isBlockquote = this.processBlockquoteFormatting(
          decorations,
          atomicDecorations,
          text,
          lineStart,
      ...
      processInlineFormatting(decorations, atomicDecorations, text, lineStart) {
        ...
        this.processBoldItalicFormatting(
          inlineDecorations,
          inlineAtomicDecorations,
          processingText,
          lineStart + processingOffset,
        );

        // Process bold formatting: **text**
        this.processBoldFormatting(
          inlineDecorations,
          inlineAtomicDecorations,
          processingText,
          lineStart + processingOffset,
        ...
        this.processUnderlineFormatting(
          inlineDecorations,
          inlineAtomicDecorations,
          processingText,
          lineStart + processingOffset,
        ...
        this.processHighlightFormatting(
          inlineDecorations,
          inlineAtomicDecorations,
          processingText,
          lineStart + processingOffset,
        ...
        this.processStrikethroughFormatting(
          inlineDecorations,
          inlineAtomicDecorations,
          processingText,
          lineStart + processingOffset,
        ...
        this.processInlineCodeFormatting(
          inlineDecorations,
          inlineAtomicDecorations,
          processingText,
          lineStart + processingOffset,
...
export const inlineFormattingStyles = EditorView.theme({
  ...
  '.cm-code-formatted': {
    ...
    fontFamily:
      'var(--mono-font-stack, SFMono-Regular, Consolas, "Liberation Mono", Menlo, monospace) !important',
  ...
  '.cm-table-formatted th.cm-table-column-selected, .cm-table-formatted td.cm-table-column-selected':
    {
      ...
      boxShadow:
        'inset 2px 0 rgba(59, 130, 246, 0.35), inset -2px 0 rgba(59, 130, 246, 0.35) !important',
    },
