...
class SlashMenuWidget {
  ...
  updateMenuContent(state) {
    ...
    const filteredCommands = filterCommands(state.filter).sort(
      (a, b) => scoreCommand(b, state.filter) - scoreCommand(a, state.filter),
    );
    ...
    filteredCommands.forEach((cmd, index) => {
      ...
      Object.assign(item.style, {
        ...
        backgroundColor:
          index === state.selectedIndex ? 'var(--bg-selected, #e8f0fe)' : 'transparent',
      });
...
function moveSelectionUp(view) {
  ...
  const newIndex = state.selectedIndex <= 0 ? filteredCommands.length - 1 : state.selectedIndex - 1;
...
function moveSelectionDown(view) {
  ...
  const newIndex = state.selectedIndex >= filteredCommands.length - 1 ? 0 : state.selectedIndex + 1;
...
export { SLASH_COMMANDS, slashMenuState, filterCommands, isInsideCode, isValidSlashPosition };
