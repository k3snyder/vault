...
import { onFileSaved } from '../app/file-open.js';
...
export class MarkdownEditor {
  constructor(container, initialContent = '') {
    ...
    if (window.perfMonitor) {
      window.perfMonitor.trackEditorMetrics(this.editorId, 'creation', creationStart);
    }

    log.debug(`📝 MarkdownEditor created with ID: ${this.editorId}`);
      ...
      class {
        ...
        build() {
          ...
          class PropsWidget extends WidgetType {
            constructor(fields, collapsed = true) {
              super();
              this.fields = fields;
            ...
            toDOM() {
              ...
              const other = Array.from(fields.keys())
              ...
              keys.forEach((k) => {
                ...
                vspan.textContent = fields.get(k);
          ...
          return Decoration.set([
            Decoration.widget({
              widget: new PropsWidget(editor.frontmatterFields, true),
              side: -1,
            }).range(0),
          ]);
        }
        update(u) {
          if (u.docChanged) this.decorations = this.build();
        }
      },
        ...
        buildDecorations(view) {
          ...
          class PropertiesHeader {
            ...
            `;
              ...
              header.setAttribute(
                'aria-label',
                `${this.collapsed ? 'Expand' : 'Collapse'} properties section`,
              );
              ...
              `;
                ...
                `;
                  ...
                  keySpan.style.cssText =
                    'font-weight: 400; color: #6b6b6b; min-width: 80px; margin-right: 8px;';
              ...
              header.addEventListener('click', (e) => {
                ...
                header.setAttribute(
                  'aria-label',
                  `${self.collapsed ? 'Expand' : 'Collapse'} properties section`,
                );
            ...
            compare(other) {
              ...
              return other instanceof PropertiesHeader && this.collapsed === other.collapsed;
            }
          }
  ...
  setupEditor(content) {
    ...
    const extensions = [
      ...
      // Keep Cmd+F on the editor for plain text find/replace
      keymap.of([
        {
          key: 'Mod-f',
          run: (view) => {
        ...
        class {
          ...
          update(update) {
            ...
            if (update.transactions.some((tr) => tr.isUserEvent('input.complete'))) {
              log.debug('WikiLink completion applied, forcing decoration update');
      ...
      keymap.of([indentWithTab, ...this.customKeymap()]),

      // Enhanced double-click word selection and triple-click paragraph selection
      EditorView.domEventHandlers({
        dblclick: (event, view) => {
          log.debug('🖱️ Double-click detected at:', event.clientX, event.clientY);

          // Get the position where the click occurred
          const pos = view.posAtCoords({ x: event.clientX, y: event.clientY });
          if (pos === null) {
            log.debug('❌ Could not get position from click coordinates');
            return false;
          }

          log.debug('📍 Click position:', pos);
          ...
          log.debug('📍 Position in line:', linePos);
          ...
          if (start < end) {
            ...
            log.debug('✅ Selecting word:', selectedText, 'from', from, 'to', to);
            ...
            setTimeout(() => {
              ...
              if (!currentSelection.empty) {
                const actualSelected = view.state.doc.sliceString(
                  currentSelection.from,
                  currentSelection.to,
                );
                log.debug('✅ Final selection:', actualSelected);
              } else {
                log.debug('❌ Selection appears to be empty');
              }
            }, 10);
          ...
          log.debug('❌ No word found at click position');
          return false;
        },
        ...
        click: (event, view) => {
          ...
          if (event.detail === 3) {
            log.debug('🖱️ Triple-click detected - selecting paragraph');
            ...
            log.debug(
              `✅ Selecting paragraph from line ${startLine} to ${endLine}`,
              'from',
              from,
              'to',
              to,
            ...
            setTimeout(() => {
              ...
              if (!currentSelection.empty) {
                ...
                log.debug('✅ Final paragraph selection:', actualSelected);
              }
            }, 10);
      ...
      this.fontSizeCompartment.of(
        this.createFontSizeTheme(window.pendingEditorSettings?.fontSize || 16),
      ),
      ...
      this.wysiwygCompartment.of(
        this.wysiwygEnabled
          ? [inlineFormattingExtension, inlineFormattingStyles, blockWidgetExtension]
          : [],
      ),
    ...
    return EditorView.theme(
      {
        '&': {
          ...
          fontFamily:
            "var(--editor-font-family, 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif)",
  ...
  createFontSizeTheme(size) {
    log.debug('Creating font size theme with size:', size);
    return EditorView.theme({
      '&': { fontSize: `${size}px` },
  ...
  handleContentChange(update) {
    ...
    if (hasOpenWikiLink) {
      log.debug('WikiLink detected - keeping auto-save at 10 seconds');
    }
  ...
  clearInitialSelection() {
    ...
    setTimeout(() => {
      ...
      log.debug('🔍 Editor initialized, double-click word selection should work now');
    }, 100);
  }
  ...
  setWysiwygMode(enabled) {
    ...
    this.view.dispatch({
      effects: this.wysiwygCompartment.reconfigure(
        enabled ? [inlineFormattingExtension, inlineFormattingStyles, blockWidgetExtension] : [],
      ),
    });
  }
  ...
  // Set font size
  setFontSize(size) {
    log.debug('MarkdownEditor.setFontSize called with size:', size);
    if (this.view) {
      log.debug('Dispatching font size change to editor view');
      try {
        this.view.dispatch({
          effects: this.fontSizeCompartment.reconfigure(this.createFontSizeTheme(size)),
        });
        log.debug('Font size change dispatched successfully');
      } catch (error) {
  ...
  refreshTheme() {
    log.debug('MarkdownEditor.refreshTheme called');
    if (this.view && this.themeCompartment) {
      try {
        ...
        log.debug('Refreshing theme with type:', themeType);

        // Force CodeMirror to recreate and apply the theme
        // This will re-read the CSS variables
        this.view.dispatch({
          effects: this.themeCompartment.reconfigure(this.createTheme(themeType)),
        });

        log.debug('Theme refresh dispatched successfully');
        // Theme refreshed
      } catch (error) {
  ...
  async save() {
    if (this.currentFile) {
      try {
        ...
        log.debug(
          `💾 Saving file: ${this.currentFile} (scroll: top=${initialScrollTop}, left=${initialScrollLeft})`,
        );
        ...
        const absolutePath =
          this.currentFile.startsWith('/') || this.currentFile.includes(':')
            ? this.currentFile
            : `${window.currentVaultPath}/${this.currentFile}`;

        log.debug('📝 Invoking write_file_content with absolute path:', absolutePath);
        ...
        try {
          ...
          log.debug('✅ write_file_content returned:', newTimestamp);
        } catch (writeError) {
        ...
        this.hasUnsavedChanges = false;
        log.debug('✅ File saved successfully, hasUnsavedChanges:', this.hasUnsavedChanges);
        ...
        log.debug('📢 Notifying tab system, file saved:', this.currentFile);
        onFileSaved(this.currentFile);
        ...
        try {
          log.debug('🔧 Calling batch_ensure_task_uuids with:', absolutePath);
          // Reuse the absolutePath we already computed above
          const taskUUIDs = await invoke('batch_ensure_task_uuids', {
            filePath: absolutePath,
          });
          log.debug('🔧 batch_ensure_task_uuids returned:', taskUUIDs);

          if (taskUUIDs && taskUUIDs.length > 0) {
            log.debug(`✅ Ensured UUIDs for ${taskUUIDs.length} tasks in ${this.currentFile}`);
          ...
          if (hasLeading) {
            ...
            if (closePos !== -1) {
              ...
              if (extra !== -1 || extraCRLF !== -1) {
                const updatedCanonical = await invoke('read_file_content', {
                  filePath: absolutePath,
                });
        ...
        if (!newTimestamp && this.view) {
          ...
          if (currentScrollTop !== initialScrollTop || currentScrollLeft !== initialScrollLeft) {
            log.debug(
              `⚠️ Scroll position changed during save without timestamp update. Restoring...`,
            );
        ...
        if (content.includes('- [ ]') || content.includes('- [x]')) {
          log.debug('[Editor] Full path for sync:', absolutePath);
  ...
  async autoSave() {
    ...
    if (hasOpenWikiLink) {
      log.debug('Skipping auto-save - WikiLink still open');
  ...
  togglePreview() {
    ...
    log.debug('Preview toggle not implemented yet');
  }
  ...
  async reset() {
    log.debug('🔄 Resetting CodeMirror editor instance');
    ...
    log.debug('✅ Editor reset complete');
  }
  ...
  async syncTasksToIndex(filePath) {
    ...
    try {
      log.debug('[Editor] Syncing tasks to index for:', filePath);
      log.debug('[Editor] Current file:', this.currentFile);
      log.debug('[Editor] Window vault path:', window.currentVaultPath);
      log.debug('[Editor] Path to sync:', pathToSync);

      const result = await invoke('sync_file_tasks_to_index', {
        filePath: pathToSync,
      });
      log.debug('[Editor] Task sync completed, result:', result);
  ...
  destroy() {
    log.debug(`🧹 Destroying MarkdownEditor instance: ${this.editorId}`);
    ...
    log.debug('✅ MarkdownEditor destroyed');
  }
}
