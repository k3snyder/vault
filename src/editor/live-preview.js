...
const log = createLogger('live-preview');
  ...
  toDOM() {
    ...
    checkbox.addEventListener('click', async (e) => {
      ...
      try {
        ...
        try {
          toast.success(e.target.checked ? 'Task marked done' : 'Task updated', 1200);
        } catch (error) {
          log.swallow('suppressed error', error);
        }
      } catch (err) {
...
export function toggleLivePreview(view, enabled) {
  ...
  if (enabled) {
    effects.push({
      effects: view.state.compartment.reconfigure([livePreviewPlugin, livePreviewStyles]),
    });
  } else {
    effects.push({
