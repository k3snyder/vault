...
const log = createLogger('tauri-invoke');
...
async function initializeInvoke() {
  ...
  log.debug('[TauriInvoke] Initializing invoke function...');
  ...
  if (!isTauriEnvironment()) {
    ...
    throw new Error(
      'Not running in Tauri window. Use "npm run tauri:dev" instead of "npm run dev"',
    );
  }
  ...
  try {
    ...
    if (tauriCore && tauriCore.invoke) {
      log.debug('[TauriInvoke] Successfully imported invoke from @tauri-apps/api/core');
  ...
  if (window.__TAURI__?.core?.invoke) {
    log.debug('[TauriInvoke] Using window.__TAURI__.core.invoke fallback');
    invokeFunction = window.__TAURI__.core.invoke;
    return invokeFunction;
  }

  // Wait a bit and try again (in case Tauri is still initializing)
  log.debug('[TauriInvoke] Tauri not ready, waiting 100ms...');
  await new Promise((resolve) => setTimeout(resolve, 100));

  if (window.__TAURI__?.core?.invoke) {
    log.debug('[TauriInvoke] Found window.__TAURI__.core.invoke after delay');
    invokeFunction = window.__TAURI__.core.invoke;
    return invokeFunction;
  }
...
export async function invokeWrapper(command, args) {
  log.debug(`[TauriInvoke] Calling ${command} with args:`, args);

  try {
    const invoke = await getInvoke();
    const result = await invoke(command, args);
    log.debug(`[TauriInvoke] ${command} completed successfully`);
    return result;
  } catch (error) {
