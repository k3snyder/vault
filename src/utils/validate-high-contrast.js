...
export function validateHighContrastMode() {
  ...
  Object.entries(COLOR_VARIANTS).forEach(([key, variant]) => {
    ...
    variantResult.tests.forEach((test) => {
      ...
      console.log(
        `    Ratio: ${test.ratio}:1 (AA: ${test.AA ? '✓' : '✗'}, AAA: ${test.AAA ? '✓' : '✗'})`,
      );
  ...
  borderResult.tests.forEach((test) => {
    ...
    console.log(
      `    Increase: ${test.increase}, In Range (0.2-0.5): ${test.inRange ? 'Yes' : 'No'}`,
    );
  ...
  console.log(`\n${focusResult.name}:`);
  console.log(
    `  ${focusColor}${focusStatus}${resetColor} Standard: ${focusResult.standard} → High Contrast: ${focusResult.highContrast}`,
  );
