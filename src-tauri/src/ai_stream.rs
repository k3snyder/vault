...
pub async fn send_ai_chat(app: AppHandle, messages: Vec<ChatMessage>) -> Result<String, String> {
    ...
    if !response.status().is_success() {
        ...
        println!("API Error - Status: {status}, Error: {error_text}");
...
pub async fn check_ollama_status(endpoint: String) -> Result<bool, String> {
    ...
    let status_url = format!("{base_url}/api/tags");
...
pub async fn send_ai_chat_with_functions(
    ...
    println!("Sending request to {url} with timeout: {timeout_duration:?}");
    if is_ollama {
    ...
    let response = match request.send().await {
        ...
        Err(e) => {
            if is_ollama && e.is_timeout() {
                return Err("Ollama request timed out after 5 minutes. The model may still be processing. Try a smaller request or faster model.".to_string());
            }
            return Err(format!("Failed to connect: {e}"));
        }
    };
