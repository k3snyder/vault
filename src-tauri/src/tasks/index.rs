    ...
    pub async fn verify_consistency(&self) -> Result<()> {
        ...
        for ids in inner.by_file.values() {
            for id in ids {
                if !inner.tasks.contains_key(id) {
                    return Err(anyhow!(
                        "Inconsistency: task {id} in by_file index but not in primary storage"
                    ));
                }
            }
        }
        ...
        for ids in inner.by_status.values() {
            for id in ids {
                if !inner.tasks.contains_key(id) {
                    return Err(anyhow!(
                        "Inconsistency: task {id} in by_status index but not in primary storage"
                    ));
                }
            }
        }
        ...
        for ids in inner.by_project.values() {
            for id in ids {
                if !inner.tasks.contains_key(id) {
                    return Err(anyhow!(
                        "Inconsistency: task {id} in by_project index but not in primary storage"
                    ));
                }
            }
        }
        ...
        for ids in inner.by_due_date.values() {
            for id in ids {
                if !inner.tasks.contains_key(id) {
                    return Err(anyhow!(
                        "Inconsistency: task {id} in by_due_date index but not in primary storage"
                    ));
                }
            }
        }
        ...
        for ids in inner.by_priority.values() {
            for id in ids {
                if !inner.tasks.contains_key(id) {
                    return Err(anyhow!(
                        "Inconsistency: task {id} in by_priority index but not in primary storage"
                    ));
                }
            }
        }
        ...
        for (id, task) in &inner.tasks {
            ...
            if let Some(file_tasks) = inner.by_file.get(&task.file_path) {
                if !file_tasks.contains(id) {
                    return Err(anyhow!("Inconsistency: task {id} not in by_file index"));
                }
            } else {
                return Err(anyhow!(
            ...
            if let Some(status_tasks) = inner.by_status.get(&task.status) {
                if !status_tasks.contains(id) {
                    return Err(anyhow!("Inconsistency: task {id} not in by_status index"));
                }
            } else {
                return Err(anyhow!(
            ...
            if let Some(project) = &task.project {
                if let Some(project_tasks) = inner.by_project.get(project) {
                    if !project_tasks.contains(id) {
                        return Err(anyhow!("Inconsistency: task {id} not in by_project index"));
                    }
                } else {
                    return Err(anyhow!(
                        "Inconsistency: project {project} not in by_project index"
                    ));
                }
            }
            ...
            if let Some(due_date) = task.due_date {
                if let Some(date_tasks) = inner.by_due_date.get(&due_date) {
                    if !date_tasks.contains(id) {
                        return Err(anyhow!("Inconsistency: task {id} not in by_due_date index"));
                    }
                } else {
                    return Err(anyhow!(
                        "Inconsistency: date {due_date} not in by_due_date index"
                    ));
                }
            }
            ...
            if let Some(priority) = &task.priority {
                if let Some(priority_tasks) = inner.by_priority.get(priority) {
                    if !priority_tasks.contains(id) {
                        return Err(anyhow!("Inconsistency: task {id} not in by_priority index"));
                    }
                } else {
                    return Err(anyhow!(
                        "Inconsistency: priority {priority:?} not in by_priority index"
                    ));
                }
            }
        }
