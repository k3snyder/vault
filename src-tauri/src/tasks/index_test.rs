...
async fn test_performance_with_many_tasks() {
    ...
    assert!(
        ...
        "Project query took {project_query_time:?}"
    );
    ...
    assert!(
        ...
        "Status query took {status_query_time:?}"
    );
    ...
    assert!(
        ...
        "Date query took {date_query_time:?}"
    );
...
async fn test_cache_hit_rate() {
    ...
    for _ in 0..10 {
        for i in 0..10 {
            let _ = index.get_task(&format!("task-{i:03}")).await;
        }
    }
