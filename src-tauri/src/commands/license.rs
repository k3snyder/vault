use crate::license::storage::{load_license_state, store_signed_license, StoredLicenseState};
...
pub async fn get_license_status() -> Result<LicenseStatusResponse, String> {
    ...
    match load_license_state(&machine_id)? {
        StoredLicenseState::Valid(license_info) => {
        ...
        StoredLicenseState::Invalid(_) => {
            return Ok(LicenseStatusResponse::from(LicenseStatus::Invalid));
        }
        StoredLicenseState::Missing => {}
    }
...
pub async fn activate_license(key: String) -> Result<LicenseStatusResponse, String> {
    ...
    // Activate online
    let activated_license = activate_online(&key, &machine_id).await?;
    let license_info = activated_license.license_info;

    if let Some(signed_license) = activated_license.signed_license {
        store_signed_license(&machine_id, &signed_license)?;
    } else {
