...
mod task_command_tests {
    ...
    async fn test_ensure_task_uuid_creates_new_uuid() {
        ...
        // Verify file exists
        assert!(file_path.exists(), "File does not exist at: {file_path:?}");
    ...
    async fn test_ensure_task_uuid_preserves_existing() {
        ...
        let file_content = format!("# Test Note\n\n- [ ] Test task <!-- tid:{existing_uuid} -->\n");
        let file_path = env.create_test_file("test.md", &file_content);
...
- [ ] Test task with metadata <!-- tid:{task_id} -->
- [x] Completed task
"#
        );
        let file_path = env.create_test_file("test.md", &file_content);
...
"#
        ...
        assert!(
            ...
            "Toggled line should contain [x], got: {toggled_line}"
        );
...
"#;
        ...
        // Batch ensure UUIDs
        let mut manager = env.identity_manager.write();
    ...
    async fn test_get_task_by_id() {
        ...
        let task_id = "01932e4a-1234-7890-abcd-ef1234567890";
        let file_content = format!(
            "# Test Note\n\n- [ ] Specific task <!-- tid:{task_id} -->\n- [ ] Another task\n"
        );
        let file_path = env.create_test_file("test.md", &file_content);

        // Get task by ID
        let manager = env.identity_manager.read();
    ...
    async fn test_find_duplicate_task_ids() {
        ...
        let file1_content =
            format!("# File 1\n\n- [ ] Task in file 1 <!-- tid:{duplicate_id} -->\n");
        let file2_content =
            format!("# File 2\n\n- [ ] Task in file 2 <!-- tid:{duplicate_id} -->\n");
    ...
    async fn test_performance_parse_large_file() {
        ...
        for i in 0..100 {
            content.push_str(&format!("- [ ] Task {i} @due:tomorrow !high #work\n"));
        }
