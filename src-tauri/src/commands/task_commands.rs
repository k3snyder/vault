...
pub async fn ensure_task_uuid(
    ...
    if let Some(content) = line_content {
        ...
        if let Some(task) = crate::identity::tasks::TaskParser::parse_line(&content, line_number) {
            ...
            let new_uuid = manager
                .generate_task_id()
                .map_err(|e| CommandError::Identity(e.context("Failed to generate UUID")))?;
            ...
            return Err(CommandError::invalid_input(format!(
                "Line {line_number} is not a task: {content}"
            )));
        }
    }
    ...
    let (uuid, manager_snapshot) = {
        let mut manager = identity_manager.write();
        let uuid = manager
            .ensure_task_id_without_index(path, line_number)
            .map_err(|e| CommandError::Identity(e.context("Failed to ensure task UUID")))?;
        (uuid, manager.clone())
    };
    manager_snapshot
        .sync_file_tasks_to_index_async(path)
        .await
        .map_err(|e| CommandError::Identity(e.context("Failed to sync task to index")))?;
...
pub async fn get_tasks_for_note(
    file_path: String,
    _identity_manager: State<'_, Arc<RwLock<IdentityManager>>>,
) -> CommandResult<Vec<TaskInfo>> {
    let path = Path::new(&file_path);

    // Read file content
    let content = std::fs::read_to_string(path).map_err(|e| CommandError::io(path, e))?;

    // Parse front matter to get task metadata
    let (front_matter, _body) = crate::identity::frontmatter::FrontMatterParser::parse(&content)
        .map_err(|e| CommandError::Identity(e.context("Failed to parse front matter")))?;
...
pub async fn toggle_task_by_id(
    ...
    let line_idx = match task_line_number {
    ...
    let final_content = if let Some(ref task) = task_found {
        ...
        crate::identity::frontmatter::FrontMatterWriter::write(&fm, &updated_body)
            .map_err(|e| CommandError::Identity(e.context("Failed to write front matter")))?
    } else {
        updated_body
    };
    ...
    let temp_file = tempfile::NamedTempFile::new_in(path.parent().unwrap_or(Path::new(".")))
        .map_err(|e| CommandError::io(path.parent().unwrap_or(Path::new(".")), e))?;

    {
        let mut file = temp_file.as_file();
        file.write_all(final_content.as_bytes())
            .map_err(|e| CommandError::io(path, e))?;
        file.sync_all().map_err(|e| CommandError::io(path, e))?;
    }
...
pub async fn update_task_properties(
    ...
    crate::identity::frontmatter::FrontMatterWriter::write_atomic(path, &fm, &body)
        .map_err(|e| CommandError::Identity(e.context("Failed to write file")))?;
    ...
    manager_snapshot
        .sync_file_tasks_to_index_async(path)
        .await
        .map_err(|e| CommandError::Identity(e.context("Failed to sync tasks to index")))?;
...
pub async fn batch_ensure_task_uuids(
    ...
    let (uuids, manager_snapshot) = {
        let mut manager = identity_manager.write();
        let uuids = manager
            .batch_ensure_task_ids(path)
            .map_err(|e| CommandError::Identity(e.context("Failed to batch ensure task UUIDs")))?;
        (uuids, manager.clone())
    };
...
pub async fn get_task_by_id(
    ...
    let task = { identity_manager.read().get_task_by_id(path, &task_id) };
    match task {
        Ok(Some(task)) => {
        ...
        Ok(None) => Ok(None),
        Err(e) => Err(CommandError::Identity(e.context("Failed to get task"))),
    }
}
...
pub async fn find_duplicate_task_ids(
    ...
    let duplicates = { identity_manager.read().find_duplicate_task_ids() };
    match duplicates {
        Ok(duplicates) => {
        ...
        Err(e) => Err(CommandError::Identity(
            e.context("Failed to find duplicates"),
        )),
    }
}
...
pub async fn add_task_uuids_to_vault(
    ...
    identity_manager: State<'_, Arc<RwLock<IdentityManager>>>,
) -> CommandResult<crate::tasks::TaskMigrationReport> {
    use crate::tasks::TaskMigrationManager;

    // Get vault root from identity manager
    let vault_root = { identity_manager.read().vault_root().to_path_buf() };
    ...
    .map_err(|e| CommandError::Identity(e.context("Migration failed")))?;
...
pub async fn rollback_task_migration(
    ...
    let migration_manager = TaskMigrationManager::new(
        identity_manager.inner().clone(),
        vault_root,
        Default::default(),
    );
    ...
    migration_manager
        .rollback(&report)
        .await
        .map_err(|e| CommandError::Identity(e.context("Rollback failed")))?;
...
pub async fn open_file_at_line(
    ...
    window
        ...
        .map_err(|e| CommandError::other(format!("Failed to emit event: {e}")))?;
