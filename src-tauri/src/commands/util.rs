...
pub fn check_command_exists(command: String) -> bool {
    ...
    if let Ok(home) = env::var("HOME") {
        ...
        path_components.push(format!("{home}/.claude/bin"));
                ...
                for entry in entries.flatten() {
                    if let Some(version) = entry.file_name().to_str() {
                        if version.starts_with("v") {
                            let nvm_bin = format!("{nvm_base}/{version}/bin");
    ...
    if result {
        println!("Command '{command}' found in PATH");
        return true;
    }
    ...
    if let Ok(home) = env::var("HOME") {
        ...
        if std::path::Path::new(&nvm_base).exists() {
            if let Ok(entries) = std::fs::read_dir(&nvm_base) {
                for entry in entries.flatten() {
                    if let Some(version) = entry.file_name().to_str() {
                        if version.starts_with("v") {
                            home_locations.push(format!("{nvm_base}/{version}/bin/{command}"));
                        }
                    }
                }
            }
        ...
        for loc in home_locations {
            if std::path::Path::new(&loc).exists() {
                println!("Command '{command}' found at: {loc}");
                return true;
            }
        }
    }
    ...
    println!("Command '{command}' not found in any known location");
    false
}
