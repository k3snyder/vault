...
impl PtyManager {
    ...
    pub async fn spawn_pty(
        ...
        audit_pty_spawn(&session_id, &options);
        ...
        if let Ok(home) = std::env::var("HOME") {
            common_paths.push(format!("{home}/.local/bin"));
            common_paths.push(format!("{home}/bin"));
            // Add common Claude CLI installation location
            common_paths.push(format!("{home}/.claude/bin"));

            // Add NVM paths - check for various Node versions
            let nvm_base = format!("{home}/.nvm/versions/node");
                    ...
                    for entry in entries.flatten() {
                        if let Some(version) = entry.file_name().to_str() {
                            if version.starts_with("v") {
                                common_paths.push(format!("{nvm_base}/{version}/bin"));
                            }
                        }
                    }
                }
        ...
        // GUI-launched app bundles often do not inherit TERM.
        // Set a sane default so full-screen/curses tools (tmux, CLI chat UIs) work.
        let term = std::env::var("TERM")
            .ok()
            .filter(|v| !v.trim().is_empty())
            .unwrap_or_else(|| "xterm".to_string());
        cmd.env("TERM", term.clone());
        println!("PTY: Using TERM: {term}");
        ...
        thread::spawn(move || {
            ...
            loop {
                match reader.read(&mut buffer) {
                    Ok(0) => {
                        // EOF
                        let _ = app_handle_clone.emit(&format!("pty:exit:{session_id_clone}"), ());
                        break;
                    }
                    Ok(n) => {
    ...
    pub async fn close_pty(&self, session_id: &str) -> Result<(), String> {
        ...
        tracing::info!(target: "audit.pty", session_id = %session_id, "pty close");
        Ok(())
    }
}

fn audit_pty_spawn(session_id: &str, options: &PtySpawnOptions) {
...
mod tests {
    ...
    #[test]
    fn spawn_logs_audit_record_without_env_values() {
        ...
        env.insert("SECRET_KEY".to_string(), "SECRET_VALUE_CANARY".to_string());
        ...
        with_default(subscriber, || audit_pty_spawn("session-1", &options));

        let output = String::from_utf8(buffer.lock().unwrap().clone()).unwrap();
        assert!(output.contains("pty spawn"));
        assert!(output.contains("/bin/echo"));
        assert!(output.contains("SECRET_KEY"));
        assert!(!output.contains("SECRET_VALUE_CANARY"));
    }
}
