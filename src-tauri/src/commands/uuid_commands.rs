...
pub async fn get_note_uuid(
    ...
    helper
        .get_note_id(path)
        .await
        .map_err(|e| CommandError::Identity(e.context("Failed to get note UUID")))
}
...
pub async fn ensure_note_uuid(
    ...
    helper
        .ensure_note_id(path)
        .await
        .map_err(|e| CommandError::Identity(e.context("Failed to ensure note UUID")))
}
...
pub async fn convert_legacy_id_to_uuid(
    ...
    helper
        ...
        .await
        .map_err(|e| CommandError::Identity(e.context("Failed to resolve ID")))
}
...
pub async fn batch_convert_ids(
    ...
    vault_id: String,
    identity_manager: State<'_, Arc<RwLock<IdentityManager>>>,
) -> CommandResult<Vec<IdConversionResult>> {
    let helper = ApiUpdateHelper::new(identity_manager.inner().clone());
...
pub async fn add_uuids_to_vault(
    ...
    let vault = vault_lock.as_ref().ok_or(CommandError::VaultNotOpen)?;
    ...
    match migration_manager.migrate().await {
        ...
        Err(e) => {
            ...
            Err(CommandError::Identity(
                e.context("Failed to add UUIDs to vault"),
            ))
        }
    }
}
