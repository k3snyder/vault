use crate::vault_settings::{normalize_image_location, normalize_image_reference};
...
impl PdfExporter {
    ...
    fn process_markdown_images(&self, markdown: &str) -> Result<String, String> {
        ...
        // Regular expression to find image embeds:
        // ![[filename.png]], ![[Files/filename.png]], or ![[Files/filename.png|640]]
        let syntax_pattern = regex::Regex::new(
            r"!\[\[([^\]|]+\.(png|jpg|jpeg|gif|webp|svg|bmp|ico))(?:\|(\d+)(?:x\d+)?)?\]\]",
        )
        ...
        for cap in syntax_pattern.captures_iter(markdown) {
            let filename = normalize_image_reference(&cap[1]);
            let width = cap.get(3).map(|value| value.as_str());

            // Handle both "Files/image.png" and bare "image.png" formats
            let image_path = if filename.contains('/') {
                self.vault_path.join(&filename)
            } else {
                self.vault_path
                    .join(normalize_image_location("Files/"))
                    .join(&filename)
            };
            ...
            if let Ok(base64_data) = self.image_to_base64(&image_path) {
                let replacement = self.build_embedded_image_markup(&filename, &base64_data, width);
                processed = processed.replace(&cap[0], &replacement);
                println!("📸 Embedded image: {filename}");
            } else {
                println!("⚠️ Could not embed image: {filename} at path: {image_path:?}");
            }
        }
        ...
        for cap in standard_pattern.captures_iter(&processed.clone()) {
            let alt_text = &cap[1];
            let image_path = normalize_image_reference(&cap[2]);
    ...
    fn build_embedded_image_markup(
        &self,
        filename: &str,
        base64_data: &str,
        width: Option<&str>,
        ...
        match width {
            Some(width) => format!(
                r#"<img src="{}" alt="{}" width="{}" />"#,
                escape_html_attribute(base64_data),
                escape_html_attribute(filename),
                width
            ),
            None => format!("![{filename}]({base64_data})"),
        }
    }
...
<body>
    ...
    // Save as .doc file
    fs::write(output_path, word_html).map_err(|e| format!("Failed to save Word document: {e}"))?;
