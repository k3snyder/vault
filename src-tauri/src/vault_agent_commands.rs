...
pub enum PathValidationError {
    ...
    EmptyPath,
    #[error("Path traversal attack detected: '..' not allowed")]
    PathTraversal,
    #[error("Absolute paths not allowed - use relative paths from vault root")]
    AbsolutePath,
    #[error("Path contains invalid characters")]
    InvalidCharacters,
    #[error("Path exceeds maximum length (1024 characters)")]
    TooLong,
    #[error("Resolved path is outside vault directory")]
    OutsideVault,
}

impl From<PathValidationError> for crate::command_error::CommandError {
                    ...
                    match validate_and_resolve_path(vault.path(), &file_path) {
                        Ok(_resolved_path) => {
                            ...
                            match vault.read_file(path) {
                                Ok(content) => {
                                    let len = content.len();
                                    println!("✅ agent_read_note read {len} characters");
...
pub async fn agent_notes_by_tag(
    ...
    let tag_normalized = if let Some(stripped) = tag.strip_prefix('#') {
        stripped
    } else {
        &tag
    };
...
mod tests {
    ...
    #[test]
    fn path_validation_error_messages_unchanged() {
        let cases = [
            (PathValidationError::EmptyPath, "Path cannot be empty"),
            (
                PathValidationError::PathTraversal,
                "Path traversal attack detected: '..' not allowed",
            ),
            (
            ...
            (
                ...
                "Path exceeds maximum length (1024 characters)",
            ),
            (
                PathValidationError::OutsideVault,
                "Resolved path is outside vault directory",
            ),
        ];
