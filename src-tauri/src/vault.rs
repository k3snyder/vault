...
impl Vault {
        ...
        while let Some(component) = components.next() {
            ...
            match std::fs::symlink_metadata(&candidate) {
                Ok(metadata) => {
                    if metadata.file_type().is_symlink() {
                        return Err(io::Error::new(
                            ...
                            format!("Symlinks are not allowed in vault paths: {relative_path:?}"),
                        ));
                    }
                ...
                Err(error) => {
                    return Err(io::Error::new(
                        error.kind(),
                        format!("Failed to inspect vault path {relative_path:?}: {error}"),
                    ));
                }
            }
        }
    ...
    pub fn resolve_input_path(&self, input_path: &Path) -> io::Result<PathBuf> {
        if looks_like_absolute_path(input_path) {
            ...
            return Err(io::Error::new(
                io::ErrorKind::PermissionDenied,
                format!("Absolute path {input_path:?} is outside the active vault"),
            ));
        }
