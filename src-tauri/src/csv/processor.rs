...
fn is_temporal_column(name: &str) -> bool {
    ...
    for temporal_name in temporal_names {
        if name == temporal_name || name.starts_with(&format!("{temporal_name}_")) {
            return true;
        }
    }
...
fn contains_word(name: &str, word: &str) -> bool {
    ...
    if name.starts_with(&format!("{word}_")) {
        return true;
    }
    ...
    if name.contains(&format!("_{word}_")) {
        return true;
    }
...
pub fn generate_description(name: &str, data_type: &DataType, role: &SemanticRole) -> String {
    ...
    let type_desc = match data_type {
        ...
        DataType::Currency { code } => return format!("{readable_name} in {code} currency"),
        DataType::Date { format } => return format!("{readable_name} date ({format})"),
        DataType::DateTime { format } => return format!("{readable_name} timestamp ({format})"),
