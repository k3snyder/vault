...
impl IdentityManager {
    ...
    pub fn ensure_note_id(&mut self, path: &Path) -> Result<String> {
        ...
        let mut existing_frontmatter = None;
        ...
        if path.exists() {
            if SidecarManager::should_use_sidecar(path) {
                ...
                // Write to front matter, preserving any existing fields.
                let mut fm = existing_frontmatter.unwrap_or_else(FrontMatter::new);
                let now = Utc::now();
                fm.id = Some(id.clone());
    ...
    pub fn update_note_path(&mut self, old_path: &Path, new_path: &Path) -> Result<()> {
    ...
    pub fn ensure_task_id(&mut self, file_path: &Path, line_number: usize) -> Result<String> {
        let task_id = self.ensure_task_id_without_index(file_path, line_number)?;
    ...
    /// Ensure a task has an ID without updating the async task index.
    /// Async command handlers use this under a short write lock and then sync the index after
    /// dropping the guard.
    pub fn ensure_task_id_without_index(
        &mut self,
        file_path: &Path,
        line_number: usize,
    ) -> Result<String> {
    ...
    pub async fn sync_file_tasks_to_index_async(&self, file_path: &Path) -> Result<()> {
        ...
        let content = std::fs::read_to_string(file_path).map_err(|e| {
            eprintln!(
                "[sync_file_tasks_to_index_async] Failed to read file: {file_path:?}, error: {e}"
            );
    ...
    pub async fn scan_vault_for_tasks_async(&self) -> Result<()> {
        ...
        for path in markdown_files {
            ...
            if self.sync_file_tasks_to_index_async(&path).await.is_ok() {
                task_count += 1;
            }
        }

        println!("    ✅ Scanned {file_count} markdown files: found tasks in {task_count} files");
        Ok(())
    }
    ...
    pub fn scan_vault_for_tasks(&self) -> Result<()> {
            ...
            .filter_map(|e| e.ok())
        {
            ...
            if self.sync_file_tasks_to_index(path).is_ok() {
                task_count += 1;
            }
        }
