...
#[test]
fn test_parse_preserves_numeric_and_version_fields() {
    let content = r#"---
weight: 1.0
version: 1.2.3
---
Content"#;
    ...
    assert_eq!(body, "Content");
    assert_eq!(fm.extra_fields.get("weight"), Some(&serde_json::json!(1.0)));
    assert_eq!(
        fm.extra_fields.get("version"),
        Some(&serde_json::Value::String("1.2.3".to_string()))
    );
}

#[test]
fn test_parse_skips_non_string_keys() {
    let content = r#"---
1: numeric-key
keep: string-key
---
Content"#;
    ...
    assert!(!fm.extra_fields.contains_key("1"));
    assert_eq!(
        fm.extra_fields.get("keep"),
        Some(&serde_json::Value::String("string-key".to_string()))
    );
}
...
Content"#;
    ...
    let fm = fm.unwrap();
    assert!(fm.extra_fields.contains_key("description"));
}
