...
impl FrontMatterParser {
    pub fn parse(content: &str) -> Result<(Option<FrontMatter>, String)> {
        ...
        if let Some(end_pos) = content[search_start..].find(pattern_start) {
            ...
            match serde_yaml::from_str::<YamlValue>(yaml_content) {
                Ok(yaml) => {
                    let front_matter = Self::yaml_to_frontmatter(&yaml)?;
    ...
    fn yaml_to_frontmatter(yaml: &YamlValue) -> Result<FrontMatter> {
        let mut fm = FrontMatter::new();

        if let YamlValue::Mapping(h) = yaml {
            for (k, v) in h {
                if let YamlValue::String(key) = k {
                    match key.as_str() {
                        ...
                        "legacy_ids" => {
                            if let YamlValue::Sequence(arr) = v {
    ...
    fn yaml_to_json(yaml: &YamlValue) -> serde_json::Value {
        match yaml {
            ...
            YamlValue::Tagged(tagged) => Self::yaml_to_json(&tagged.value),
        }
    }
}
