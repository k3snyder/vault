...
#[tokio::test]
async fn test_migration_writes_no_original_path_fields() {
    let temp_dir = TempDir::new().unwrap();
    let vault_root = temp_dir.path().to_path_buf();
    ...
    let identity_manager = Arc::new(RwLock::new(IdentityManager::new(vault_root.clone())));
    let migrator = MigrationManager::new(
        identity_manager,
        vault_root,
        MigrationConfig {
            show_progress: false,
    ...
    let legacy_ids = mapper::LegacyIds {
        absolute_path_hash: "absolute-hash".to_string(),
        relative_path_hash: "relative-hash".to_string(),
        original_absolute_path: "/vault/note.md".to_string(),
        original_relative_path: "note.md".to_string(),
    };

    migrator
        .store_legacy_ids(&note_path, "note-uuid", &legacy_ids)
        .await
        .unwrap();

    let written = fs::read_to_string(&note_path).unwrap();
    assert!(written.contains("legacy_ids:"));
    assert!(!written.contains("original_absolute_path"));
    assert!(!written.contains("original_relative_path"));

    let (front_matter, _body) =
        crate::identity::frontmatter::FrontMatterParser::parse(&written).unwrap();
    assert_eq!(
        front_matter.unwrap().legacy_ids,
        Some(vec![
            "absolute-hash".to_string(),
            "relative-hash".to_string()
        ])
    );
}
...
async fn test_migration_concurrent_processing() {
    ...
    for i in 0..10 {
        fs::write(vault_root.join(format!("note{i}.md")), format!("Note {i}")).unwrap();
    }
