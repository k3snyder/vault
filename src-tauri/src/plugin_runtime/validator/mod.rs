...
impl PluginValidator {
    ...
    pub async fn check_api_compatibility(
        ...
        if !is_compatible {
            breaking_changes.push(format!(
                "Plugin requires API version {required_version} but current version is {current_version}"
            ));
        }
    ...
    pub async fn analyze_performance(
        ...
        if bundle_size_kb > 1000 {
            issues.push(PerformanceIssue {
                ...
                message: format!("Bundle size is {bundle_size_kb}KB, consider optimizing"),
    ...
    pub async fn generate_json_report(
        ...
        serde_json::to_value(report).map_err(|e| ValidationError::ValidationFailed(e.to_string()))
    }
