...
#[tokio::test]
async fn test_security_scan_does_not_recurse_subdirectories() {
    let temp_dir = TempDir::new().unwrap();
    let plugin_path = temp_dir.path();

    fs::create_dir_all(plugin_path.join("src/nested")).unwrap();
    fs::write(
        plugin_path.join("src/nested/evil.js"),
        "export function run() { eval('console.log(1)'); }",
    )
    .unwrap();

    let validator = PluginValidator::new();
    let report = validator.analyze_security(plugin_path).await.unwrap();
...
#[tokio::test]
async fn test_risk_score_is_ten_per_violation() {
    let temp_dir = TempDir::new().unwrap();
    let plugin_path = temp_dir.path();

    fs::create_dir_all(plugin_path.join("src")).unwrap();
    fs::write(
        plugin_path.join("src/main.js"),
        r#"
            eval('console.log(1)');
            const fs = require('fs');
            document.body.innerHTML = userInput;
...
#[tokio::test]
async fn test_find_line_number_reports_first_occurrence_only() {
    let temp_dir = TempDir::new().unwrap();
    let plugin_path = temp_dir.path();
...
#[tokio::test]
async fn test_disable_rule_matrix() {
    let cases = [
        ("no-require", "const fs = require('fs');"),
        ("no-inner-html", "document.body.innerHTML = userInput;"),
        (
    ...
    for (rule, code) in cases {
        ...
        fs::create_dir_all(plugin_path.join("src")).unwrap();
        fs::write(plugin_path.join("src/main.js"), code).unwrap();

        let mut validator = PluginValidator::new();
        validator.disable_rule(rule);
        let report = validator.analyze_security(plugin_path).await.unwrap();

        assert!(report.is_safe, "{rule} should be disabled");
        assert_eq!(report.violations.len(), 0);
    }
}
...
#[tokio::test]
async fn test_semver_prerelease_rejected() {
    let temp_dir = TempDir::new().unwrap();
    let plugin_path = temp_dir.path();
    ...
    let validator = PluginValidator::new();
    let report = validator.validate_manifest(plugin_path).await.unwrap();

    assert!(!report.is_valid);
    assert!(report
        .errors
        .iter()
        .any(|error| error.rule == "manifest-version-format"));
}

#[tokio::test]
async fn test_version_compat_ignores_patch() {
    let temp_dir = TempDir::new().unwrap();
    let plugin_path = temp_dir.path();
    ...
    let report = validator
        .check_api_compatibility(plugin_path, "1.2.0")
        .await
        .unwrap();
...
#[tokio::test]
async fn test_version_compat_higher_major_passes() {
    let (_temp_dir, plugin_path) = helpers::create_test_plugin();
    let validator = PluginValidator::new();
    let report = validator
        .check_api_compatibility(&plugin_path, "2.0.0")
        .await
        .unwrap();
...
async fn test_api_usage_validation() {
    ...
    let report = result.unwrap();
    assert!(!report.deprecated_apis.is_empty());
    assert!(report
        .deprecated_apis
        .iter()
...
#[tokio::test]
async fn test_validate_plugin_score_arithmetic() {
    let temp_dir = TempDir::new().unwrap();
    let plugin_path = temp_dir.path();

    let manifest = serde_json::json!({
        "name": "Invalid Unsafe Plugin",
        "version": "1.0.0",
        "description": "Plugin with manifest and security issues",
        "author": "Test",
        "minApiVersion": "1.0.0",
        "permissions": [],
        "entryPoint": "dist/main.js"
    ...
    let validator = PluginValidator::new();
    let report = validator.validate_plugin(plugin_path).await.unwrap();
