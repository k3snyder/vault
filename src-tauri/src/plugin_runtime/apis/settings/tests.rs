...
mod settings_api_tests {
    ...
    mod storage_quota {
        ...
        async fn test_quota_enforcement() {
            ...
            let _ = settings.set_quota("test-plugin", 1024).await;
        ...
        async fn test_quota_cleanup() {
            ...
            let _ = settings.set_quota("test-plugin", 100).await;
            settings
    ...
    mod encryption {
        ...
        async fn test_encrypted_storage() {
            let (settings, _temp) = create_test_settings().await;
            let plugin_id = unique_plugin_id("encrypted-storage");
            grant_settings_permission(&settings, &plugin_id, SettingsPermission::Write).await;
            grant_settings_permission(&settings, &plugin_id, SettingsPermission::Read).await;

            // Store sensitive data encrypted
            let secret = "super-secret-api-key";
            let result = settings.set_encrypted(&plugin_id, "api_key", secret).await;
            assert!(result.is_ok());

            // Retrieve and decrypt
            let decrypted = settings.get_encrypted(&plugin_id, "api_key").await;
            assert!(decrypted.is_ok());
            assert_eq!(decrypted.unwrap(), Some(secret.to_string()));

            // Verify it's actually encrypted on disk
            let raw = settings
                .get_raw_internal(&plugin_id, "api_key")
                .await
                .unwrap();
        ...
        #[tokio::test]
        async fn test_encrypted_storage_survives_new_instance() {
            let temp_dir = TempDir::new().unwrap();
            let storage_path = temp_dir.path().to_path_buf();
            let plugin_id = unique_plugin_id("encrypted-restart");
            let secret = "super-secret-api-key";

            {
                ...
                grant_settings_permission(&settings, &plugin_id, SettingsPermission::Write).await;
                settings
                    .set_encrypted(&plugin_id, "api_key", secret)
                    .await
                    .unwrap();
            }

            {
                ...
                grant_settings_permission(&settings, &plugin_id, SettingsPermission::Read).await;
                let decrypted = settings.get_encrypted(&plugin_id, "api_key").await.unwrap();
        ...
        async fn test_encryption_key_rotation() {
            let (settings, _temp) = create_test_settings().await;
            let plugin_id = unique_plugin_id("key-rotation");
            grant_settings_permission(&settings, &plugin_id, SettingsPermission::Write).await;
            grant_settings_permission(&settings, &plugin_id, SettingsPermission::Read).await;

            // Store encrypted data
            settings
                .set_encrypted(&plugin_id, "secret", "my-secret")
                .await
                .unwrap();

            // Rotate encryption key
            settings.rotate_encryption_key(&plugin_id).await.unwrap();

            // Should still be able to decrypt
            let decrypted = settings.get_encrypted(&plugin_id, "secret").await.unwrap();
    ...
    mod cleanup {
        ...
        async fn test_garbage_collection() {
            ...
            let _ = settings.mark_plugin_uninstalled("plugin-1").await;
            let _ = settings.mark_plugin_uninstalled("plugin-3").await;
    ...
    mod persistence {
        ...
        #[tokio::test]
        #[ignore = "stale - tracked in RUST_TEST_QUARANTINE.md for Task 0.2"]
