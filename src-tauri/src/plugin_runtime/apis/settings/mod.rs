...
// Per-plugin storage keys are derived from an app-local salt; Vault deliberately
// never touches the OS keychain.
const PLUGIN_STORAGE_KEY_SALT: &[u8] = b"vault_plugin_storage_v1";

type SettingsMigrationTransform =
    Box<dyn Fn(HashMap<String, String>) -> HashMap<String, String> + Send + Sync>;
...
pub struct Migration {
    ...
    pub to_version: u32,
    pub transform: SettingsMigrationTransform,
}
...
impl SettingsApi {
    ...
    fn get_plugin_storage_path(&self, plugin_id: &str) -> PathBuf {
        self.storage_path.join(format!("{plugin_id}.json"))
    }
    ...
    /// Get the encryption key for a plugin (derived, never stored).
    async fn get_encryption_key(&self, plugin_id: &str) -> Result<Vec<u8>, SettingsError> {
        let mut keys = self.encryption_keys.write().await;

        if let Some(key) = keys.get(plugin_id) {
            return Ok(key.clone());
        }

        let key = crate::secrets::derive_key(PLUGIN_STORAGE_KEY_SALT, plugin_id).to_vec();
        keys.insert(plugin_id.to_string(), key.clone());
        Ok(key)
    }
    ...
    pub async fn rotate_encryption_key(&self, plugin_id: &str) -> Result<(), SettingsError> {
        ...
        for key in storage.encrypted_data.keys() {
