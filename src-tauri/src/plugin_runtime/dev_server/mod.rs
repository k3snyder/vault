...
impl DevServer {
        ...
        if src_path.exists() {
            ...
            while let Some(entry) = entries.next_entry().await? {
                ...
                if path.extension().and_then(|e| e.to_str()) == Some("ts") {
                    ...
                    let js_path = dist_path.join(format!("{file_name}.js"));
                    ...
                    if source_maps {
                        let map_path = dist_path.join(format!("{file_name}.js.map"));
    ...
    async fn setup_file_watcher(&self, plugin_path: &Path) -> Result<(), DevServerError> {
        ...
        let mut watcher =
            ...
            .map_err(|e| DevServerError::Io(std::io::Error::other(e)))?;
    ...
    async fn start_http_server(
        ...
        *self.server_handle.lock().await = Some(handle);
        ...
        Ok(ServerInfo {
            ...
            api_url: format!("http://localhost:{port}/api"),
