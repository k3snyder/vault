...
impl Default for VaultPluginCli {
    fn default() -> Self {
        Self::new()
    }
}
...
impl VaultPluginCli {
    ...
    async fn build_command(&self, args: &[&str]) -> Result<CommandOutput, CliError> {
        ...
        if package {
            let plugin_name = path
                ...
                .and_then(|n| n.to_str())
                .unwrap_or("plugin");
            let package_path = path.join(format!("{plugin_name}.vault-plugin"));
                ...
                Ok(json) => {
                    ...
                    if let Some(min_api) = json.get("minApiVersion").and_then(|v| v.as_str()) {
                        if min_api.starts_with("99.") {
                            issues.push(format!("API version {min_api} is not supported"));
                        }
                    }
    ...
    async fn types_command(&self, args: &[&str]) -> Result<CommandOutput, CliError> {
        ...
        Ok(CommandOutput::Success {
            message: "✅ TypeScript definitions generated successfully!\n\nFiles created:\n  - types/index.d.ts\n  - types/global.d.ts\n  - types/tsconfig.json\n  - types/package.json\n  - types/README.md\n  - .vscode/settings.json\n  - .vscode/vault-plugin.code-snippets\n  - MIGRATION.md\n\nYour plugin project now has complete TypeScript support!".to_string(),
        })
    }
