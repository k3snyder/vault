...
impl PermissionManager {
    ...
    pub async fn persist_permissions(&self, plugin_id: &str) -> Result<(), PermissionError> {
        ...
        // Write to file
        let file_path = permissions_dir.join(format!("{plugin_id}.json"));
    ...
    pub async fn check_path_permission(
        ...
        Err(PermissionError::Denied(format!(
            "Plugin {plugin_id} lacks {permission_type:?} permission for path: {path}"
        )))
    }
    ...
    pub async fn check_network_permission(
        ...
        Err(PermissionError::Denied(format!(
            "Plugin {plugin_id} lacks network permission for URL: {url}"
        )))
    }
