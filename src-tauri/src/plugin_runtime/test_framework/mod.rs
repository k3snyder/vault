...
type ScheduledTask = (u64, Box<dyn FnOnce() + Send>);
    ...
    pub async fn advance(&mut self, ms: u64) {
        if matches!(self.mode, TimeControl::Manual | TimeControl::Accelerated(_)) {
            ...
            *self.current_time.write().await = new_time;
            ...
            let to_execute: Vec<ScheduledTask> = Vec::new();

            tasks.retain(|(time, _task)| *time > new_time);
        }
    }
...
impl Assertions {
    ...
    pub fn assert_not_equals<T: PartialEq + std::fmt::Debug>(
        ...
        if actual == expected {
            panic!("Assertion failed: {message} - values should not be equal: {actual:?}");
        }
    }
    ...
    pub fn assert_greater_than<T: PartialOrd + std::fmt::Debug>(
        ...
        if actual <= expected {
            panic!("Assertion failed: {message} - {actual:?} should be greater than {expected:?}");
        }
    }

    pub fn assert_contains(&self, haystack: &str, needle: &str, message: &str) {
        if !haystack.contains(needle) {
            panic!("Assertion failed: {message} - '{haystack}' should contain '{needle}'");
        }
    }

    pub fn assert_true(&self, value: bool, message: &str) {
        if !value {
            panic!("Assertion failed: {message} - expected true, got false");
        }
    }

    pub fn assert_false(&self, value: bool, message: &str) {
        if value {
            panic!("Assertion failed: {message} - expected false, got true");
        }
    }

    pub async fn assert_file_exists(&self, path: &str) {
        if self.vault.read_file(path).await.is_err() {
            panic!("Assertion failed: File '{path}' does not exist");
        }
    }
    ...
    pub async fn assert_file_content(&self, path: &str, expected: &str) {
        match self.vault.read_file(path).await {
            Ok(content) => {
                if content != expected {
                    panic!("Assertion failed: File '{path}' content mismatch");
                }
            }
            Err(_) => panic!("Assertion failed: File '{path}' not found"),
        }
    }

    pub async fn assert_file_not_exists(&self, path: &str) {
        if self.vault.read_file(path).await.is_ok() {
            panic!("Assertion failed: File '{path}' should not exist");
        }
    }
}
...
impl TestDataGenerator {
    ...
    pub fn generate_file_structure(&mut self, count: usize) -> Vec<String> {
        (0..count).map(|i| format!("file{i}.md")).collect()
    }
}
